...
- 鹈鹕详情恢复默认完整画面。桌面与 393 px 手机均验证：打开时没有 fit-window，切换适应窗口后关闭再打开仍回到完整画面。总览自动播放及无停止预览按钮的行为保留。加载提示改为允许点击穿透，加载过程中也能打开作品。
- 桌面与手机导航一致：云计算 / 人工智能 / 鹈鹕测试 / 编年史 / 关于。移除独立的更新记录导航。
- 原更新记录正文并入关于页的 `#updates`，保留日期与核验范围。逐条核对 79 条原表格行及来源行全部保留；文章核验链接直达新位置。
- 原 `/updates` 自动跳转到 `/about#updates`；带原章节锚点的链接保留定位，逐页处理范围实测在视口内。搜索“更新记录”只返回关于页及其章节，重定向页不再进入搜索清单。
- 22 项测试、原始作品校验、正式 base 构建（最终 31.48 秒）及 diff 检查通过。浏览器无 error / warning。独立输出目录避免影响其他预览；原有构建警告不变。
- 截图位于 `design/pelican-lab/qa/`：`navigation-mobile.png`、`full-detail-mobile.png`、`about-updates.png`。无横向溢出或阻塞问题。

## 鹈鹕展区自动播放调整（2026-09-26）

final result: passed

按用户后续要求，总览可见作品默认同时播放；滚动到新的作品后自动挂载动画，离屏、后台或打开详情时释放总览 iframe，回到总览自动恢复。移除展区的播放 / 停止预览控件，点击整个动画或「放大查看」仍可进入详情。详情每次打开默认适应窗口，保留「恢复完整画面」；模型原输出内自带的控件不改写。

- 桌面首屏无需点击即加载 3 个动画；滚至底部自动运行后续 4 个，原首屏 iframe 已释放。读取 qwen3.8-max 的 SVG 变换，前后数值不同，确认实际运动。
- 详情默认 `.fit-window` 生效，工具栏只有视口切换和前后作品；打开详情时总览 iframe 为 0，关闭后可见作品恢复。手动切至完整画面后关闭再打开，仍默认适应窗口。
- 393 × 852 手机无页面横向溢出，详情 iframe 宽 393 px，自适应状态正常。部分原始 SVG 使用 slice 或固定尺寸，适应窗口时可能按原实现裁切；完整画面切换仍可使用，源文件保持不变。
- medium 筛选得到 2 个作品，两个都自动播放；刷新带筛选 URL 后仍为 2 个动画，无浏览器 error / warning。
- 22 项既有自动测试、数据校验、正式 base 构建和 diff 检查通过。为避免其他任务同时重建共用 dist，最终使用隔离工作目录复核，构建用时 12.80 秒。
- 截图：`design/pelican-lab/qa/autoplay-desktop.png`、`autoplay-detail.png`。保留布局、字体、颜色与原始作品，仅调整播放默认值、默认视口及相关控件。
