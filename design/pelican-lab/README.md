...
新增顶层导航「鹈鹕测试」，放在「编年史」之前，进入独立的宽版三列画廊；在首页现有鹈鹕动画下方补一个「同一道题，看看其他模型怎么画 →」链接。
...
导航顺序：云计算 / 人工智能 / **鹈鹕测试** / 编年史 / 关于（含更新记录）。
...
总览中进入视口的作品默认自动播放，可同时浏览多张动画；离屏时释放 iframe，滚回后自动恢复。点击动画或「放大查看」进入详情，详情默认展示完整画面，可按需切换「适应窗口」。
