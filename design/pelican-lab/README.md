...
卡片由 16:10 预览区、模型全名、effort 标签及「播放」「放大查看」组成。日期和批次可以在详情显示，数据存在时才呈现。模型名最多两行，不让关键后缀消失。
...
默认封面静止；点击播放后挂载对应原始 HTML 预览。初版同时播放一张，切换作品停止上一张。可以随后增加「播放本行」，上限三张。

离屏、切走页面、关闭放大层、切换筛选后，停止对应预览并释放 iframe。外层控件称「停止预览 / 重新播放」：不同输出使用 CSS、SMIL、JavaScript，不能假定一个 CSS 开关就能暂停全部动画。再次播放允许从头开始。原始页面自带的暂停、速度和车铃等控件可在放大层操作。

加载状态要有文字；失败则显示「预览未能加载」和重试，保留模型信息，不显示虚构封面。所有播放操作可通过键盘完成；图片提供模型与 effort 的替代说明。系统要求减少动画时仍从静态封面开始。
