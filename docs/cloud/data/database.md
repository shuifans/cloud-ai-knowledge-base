...
reviewScope: 半同步提交顺序、副本与备份、DynamoDB 一致性与 CAP 边界
outline: [2, 3]
---

# 数据库选型

![数据库内核骨架、架构演进、形态决策与迁移阶段](/images/cloud/data/database/database-overview.png)

*本站生成的高清全文阅读地图；具体版本、参数与结论以正文引用的一手来源为准。*

> 数据库是大多数系统里最难回退的技术决策：代码换框架是重构，数据换引擎是搬家。这篇面向要做选型、或者正在为"当年随手选的库"还债的工程师和架构师，沿一条主线讲透六件事：**单机内核机制（B+ 树 / LSM-Tree / MVCC / 隔离级别）、云数据库架构演进的三级跳（主备 → 存算分离 → 云原生分布式）、Serverless 数据库的弹性与计费真相、多模谱系与向量检索现状、迁移上云的四阶段方法论、以及一线踩过的坑怎么绕开**。读完你应该能回答：业务在选型三角里牺牲什么、保住什么；以及在"集中式云原生 / 分布式 / Serverless"三种形态之间，负载该站哪边。

## 是什么：一张版图和一场迁移

先给数据库选型下一个工程定义：**在访问模式、一致性、规模、成本四个约束下，为业务数据选择一个“运维得起”的存储引擎组合。** 注意是组合——中大型系统通常需要按访问模式拆分存储职责，很少由一种数据库覆盖全部需求。

DB-Engines 是综合搜索、招聘与社区信号的流行度排行，不是安装量或收入份额，也不能据分数变化断言某数据库已取代另一数据库。本文按数据模型、事务语义与工作负载组织选型，不沿用旧榜单分数作为当前市场结论。
...
最后这一类是近三两年才挤进选型桌的：大模型应用落地让"向量相似度检索"从冷门能力变成了标配需求。工程判断是：**向量库短期内不会取代任何一类数据库，但会长期寄生在它们身上**——中小规模场景 pgvector 这类扩展够用，十亿级向量、高 QPS 才值得上 Milvus 这类专用引擎。
...
由此可得出实操判断：**数据库性能问题常与访问模式和索引结构不匹配有关**。buffer pool 命中率掉下来（数据量超过内存），延迟立刻从微秒级跳到毫秒级——云上扩容第一反应应该是加内存规格，而不是加计算核数。

buffer pool 命中率是应优先观察的一个指标，MySQL 侧的口径：
...
阵营归属上：InnoDB、PostgreSQL 默认索引属于 B-tree 家族；RocksDB/LevelDB、TiKV、Cassandra、HBase 等主要采用 LSM-tree。MongoDB 默认的 WiredTiger 则以 B-tree 组织表与索引，并非 MongoDB 默认采用 LSM-tree（[WiredTiger B-Trees](https://source.wiredtiger.com/develop/arch-btree.html)）。工程判断：**写入吞吐与顺序追加为主的负载（时序、消息、事件流）选 LSM 系；点查 + 范围查混合、延迟 P99 敏感的 OLTP 选 B+ 树系**；在云盘（本质是分布式块存储、IOPS 与吞吐解耦）上，LSM 的顺序写优势比本地盘时代小了一些，但 compaction 对 IOPS 的吞噬依然是 LSM 系实例规格规划的第一约束。
...
经验建议：**许多互联网业务可从 RC 起步，再按一致性要求评估 RR 或 SERIALIZABLE**，真正需要 RR/SERIALIZABLE 的是账务核对类路径，用显式锁或单事务内完成比抬全局隔离级别便宜。从 MySQL 迁 PG/Oracle 系时，默认隔离级别从 RR 掉到 RC 带来的行为差异（同一事务内两次读结果不同），是回归测试必须覆盖的用例。
...
- **只读副本不是备份**：误 `DROP TABLE` 会忠实地同步到所有从库。副本不防逻辑误操作；保留独立备份并结合适当日志，才能支持时间点恢复（PITR）。

半同步复制的延迟账值得单独算一笔，因为它直接决定你的 P99：

- **异步**：主库本地 fsync 完就返回，从库收没收到不管——丢数据窗口敞开。
- **半同步 AFTER_COMMIT**：源端先提交存储引擎，再等待副本 ACK，之后才向客户端返回；等待期间其他会话可能已看到该事务。
- **半同步 AFTER_SYNC**：先写并同步 binlog，等待副本 ACK，再提交存储引擎并响应客户端。ACK 表示副本已接收并持久化到 relay log，不代表已经应用。安全切换仍依赖选中正确副本、补齐日志和防止旧主继续写；超时退为异步时不再有同样保证。[MySQL 官方等待点说明](https://dev.mysql.com/doc/refman/8.0/en/replication-options-source.html)（2026-09-26）
...
- Cassandra 可配置一致性级别；Dynamo 论文的设计不能直接代表今天的 DynamoDB。DynamoDB 表与 LSI 支持可选强一致读，GSI/Streams 不支持；全局表还区分 MREC 与 MRSC。需按具体模式、地域与限制核对。[DynamoDB 一致性文档](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html)（2026-09-26）
- TiDB/CockroachDB 这类 NewSQL 选择 CP：Raft/Paxos 多数派复制，分区时宁可拒绝服务也不返回旧数据。

**把一致性要求落实到具体操作**：哪些读需要读己之写，哪些写必须跨分片原子提交，网络分区时允许哪些操作失败。CAP 中的 C 指线性一致性、A 指非故障节点请求最终获得响应，不等于 SQL 的 ACID 或商业 SLA，不能只给产品贴 CP/AP 标签。
...
工程判断：**负载曲线峰谷比大于 5 倍、或存在大量"开着没人用"的实例（开发测试环境是重灾区），Serverless 是净省钱；峰谷比小于 2 倍的稳态负载，预置规格 + 预留实例更便宜**。先把监控里的 CPU/连接数曲线画出来再决定，别先决定再找理由。
...
**向量能力现状（截至 2026-09）**：关系型数据库正在补齐向量类型和检索能力——PostgreSQL 生态的 [pgvector](https://github.com/pgvector/pgvector) 已进入 0.8 系，支持迭代式索引扫描；MySQL 9.x 引入 [VECTOR 类型](https://dev.mysql.com/doc/refman/9.7/en/vector.html)，但官方文档明确其向量列不能作为键，不能据此等同于完整的 ANN 索引能力；Oracle Database 23ai 提供 [AI Vector Search](https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/ai-vector-search-users-guide.pdf)；SQL Server 2025 加入[原生 VECTOR 类型](https://learn.microsoft.com/en-us/sql/t-sql/data-types/vector-data-type?view=sql-server-ver17)，而 DiskANN 索引等能力仍需核对版本与预览开关。专用引擎（Milvus、Qdrant 类）则面向超大规模、高 QPS 和多租户过滤检索。HNSW 是常见 ANN 索引之一：多层小世界图由稀疏上层路由到稠密下层精查，在典型数据集上可显著减少候选遍历；复杂度与召回率受图参数和数据分布影响，并无统一的严格对数保证。
...
更稳妥的判断是：**中小规模且过滤条件与业务表强耦合的场景，直接用业务库内建向量（少一个系统、少一次同步）；超大规模、或召回质量与 QPS 是产品核心指标的场景，上专用引擎**。向量索引只是 RAG 链路的一环，召回-重排-生成的整体设计见 [RAG 架构](/ai/application/rag-architecture)。
...
建议的原则是：**能不拆就不拆；要拆先拆"读"（读写分离、缓存），再拆"写"（分库分表）**。分库分表的真实成本在拆分之后才显现：跨分片 JOIN 与聚合要业务层改写、分页变成归并、全局唯一 ID 要发号器、扩容涉及数据搬迁、热点分片（某个大商家占满一个分片）需要二次打散。分片键一旦选定就很难更换；常见错误是选择"查询最方便的列"，而没有验证写入分布是否均匀。
...
备份恢复演练是被低估最严重的一项：基线要求是**核心库每季度一次真实恢复演练**（拉快照到新实例 + 校验 + 计时），演练记录进 runbook；没演练过的备份等于没有备份。PITR（按时间点恢复）要验的最坏情况不是"恢复到昨天"，而是"误删表后 5 分钟发现、要求恢复到误删前 10 秒"。
...
DTS 类工具的机制是这套流程的物理基础：结构迁移（建表/索引/约束）→ 全量迁移（按主键切片并行搬运）→ 增量迁移（解析源库 binlog/redo 日志持续回放）→ 追平至秒级延迟后达到可割接状态 → 一致性校验作为门禁 → 反向同步链路预建好供回滚。关键点：**增量追平不等于可以割接，校验通过才是门禁**；常见案例是在追平后割接，三天后发现某类 DDL 事件在增量链路里被跳过导致两库结构分叉的事故——割接前核对结构版本与校验报告，比盯延迟曲线重要。
...
迁移流程可进一步细化为：**全量 → 增量追平 → 一致性校验**（DTS 类工具的标准流程，校验不是可选项，是切换门禁）。切换阶段用"双写灰度"：新库影子写 → 比对无误后读切新库 → 写切新库 → 保留旧库反向同步作为回滚通道。整个过程的每一步都必须有预案好的回退动作。切换瞬间的窗口期约束（停写、或双写冲突仲裁）要提前和业务方签字确认，这是流程问题不是技术问题。
