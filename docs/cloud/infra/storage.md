...
reviewScope: EBS 快照删除、S3 前缀与条件写语义；其余报价保留原快照日期
outline: [2, 3]
---

# 云存储

![块存储、文件存储与对象存储的选择树及数据保护能力](/images/cloud/infra/storage/storage-overview.png)

*本站生成的高清全文阅读地图；具体版本、参数与结论以正文引用的一手来源为准。*

> 存储是方案设计中最不应被推迟考虑的一层：计算可以换、网络可以改，数据一旦落下去，搬家成本就是量级差。这篇面向要做选型、降本或排查 IO 问题的工程师与架构师，按一条主线展开：**块/对象/文件三种形态在机制层面的分界（一致性、元数据、扩展上限、协议）→ 每种形态内部是怎么实现持久性与性能的（三副本与纠删码的冗余账、快照增量链、元数据分区、并行文件系统的 MDS/OST）→ 存算分离与 AI 时代对存储的新要求 → 一线最容易踩的坑**。读完你应当能独立回答"这个负载该用什么存储、买多大性能、缓存加在哪、怎么备份"这四个问题。文中的规格数字均为各厂商公开文档口径，截至 2026-09。
...
三者可这样区分：**块是"一块还没格式化的硬盘"，文件是"一个多个机器共用的目录"，对象是"一个用 HTTP 访问的无限大 K/V 仓库"**。形态决定了语义：块语义才有真正的随机覆写和文件系统锁；文件语义天然多客户端共享；对象语义只有"整体写入、整体读取"，改一个字节也要重传整个对象，"目录"只是 key 上的前缀假象。
...
- **成本大头藏在 IO 和流量里**。存储账单分析中：存储本体单价便宜，但"买了容量没买够 IOPS 被迫整机升配""对象存储被当盘挂导致请求费失控""快照只增不减"这三类浪费非常普遍。
...
- **存储层是性能事故的背锅位与真凶位**。常见的“变慢”投诉中，根因在存储侧（突发额度、快照懒加载、元数据风暴、跨 AZ 流量、热前缀限流）的比例，远高于排查开始时大家愿意相信的比例；把存储机制读懂，等于给团队的 MTTR 买保险。
...
本地 SAN/文件系统时代的快照有两条实现路线：**COW（Copy-on-Write）**——首次覆写某旧块前，先把旧数据拷贝到快照空间，再原地覆写，写路径多一次读+写；**ROW（Redirect-on-Write）**——新写落到新块，元数据指针改指新块，快照保留旧指针，写路径干净但产生碎片。云盘快照几乎都做成第三种形态：**块粒度的增量快照 + 对象存储底座**：第一次全量，之后每次只把变化块上传到对象存储，快照之间形成引用链。以下以 EBS 为例说明；其他产品需分别核对删除、归档与恢复语义：

1. **EBS 删除旧快照不会破坏后续快照恢复**：服务会保留仍被其他快照引用的数据块，只回收独占块；因此删除后费用也不一定立刻下降。没有“必须从新到旧删除、必须保留链头”的通用规则。归档、保留锁及 AMI 关联另有约束，应分别核对。[AWS 删除快照规则](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-deleting-snapshot.html)（2026-09-26）
...
Bucket / Object / Prefix 是对象存储的全部概念模型——**没有真正的目录**，`a/b/c.jpg` 只是一个 key 里带了两个斜杠。列举"目录"靠 delimiter 参数做前缀匹配，rename 大"目录"实际是 copy+delete 遍历每个对象。这是常见的、从块存储背景转过来的同事最容易低估的一条语义差。
...
**为什么 S3 API 成了事实标准**？可归纳为是三条：接口面小且十年稳定（PUT/GET/DELETE/LIST + 分片上传 + range 读，一只手数得完）；生态先绑死（Hadoop S3A、Spark、TensorFlow/PyTorch 的 IO 层、主流备份与 CDN 产品都原生说 S3）；兼容实现多（Ceph RADOSGW、MinIO、各家云对象存储都提供 S3 兼容端点）。结果是"面向 S3 API 写代码"等于"面向所有对象存储写代码"，这条护城河比任何单点性能都深。OSS 的 API 与 S3 高度兼容但仍有细节差异（如部分头、事件通知语义），跨云代码要留兼容层测试。
...
两个容易被漏掉的暗账：一是**取回费 + 不足时长补差价**，低频/归档的"便宜"只在数据真的冷且存够时长时成立；二是**解冻配额**。按阿里云 [RestoreObject 官方文档](https://help.aliyun.com/zh/oss/developer-reference/restoreobject) 的参考值，单账号、单地域的冷归档总解冻配额为每天 100~120 TB，深度冷归档为每天 10~15 TB；超出后仍可提交，但会排队且可能超过目标解冻时长。灾备设计必须按实际存储级别核算，不能混用两个档位。
...
**key 布局应服务于访问与治理，而不是机械加盐。** S3 会自动扩展请求容量，高吞吐负载可以通过多前缀并行分散压力，并对 503 做退避；不能断言日期前缀必然落到同一物理分区。哈希前缀是一种选择，会增加按日期列举与生命周期管理的复杂度。4 位十六进制有 65,536 种可能，也不等于后端一定创建同样多的分区。S3 Express 目录桶另有规则，官方明确不建议为了熵值给目录前缀加随机字符。[S3 性能指南](https://docs.aws.amazon.com/AmazonS3/latest/userguide/optimizing-performance.html) · [目录桶指南](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-express-optimizing-performance-design-patterns.html)（2026-09-26）

#### 强一致不等于并发写不会覆盖

对 S3 单对象写入，可用 `If-None-Match: *` 实现“只在当前对象不存在时创建”，用 `If-Match: <ETag>` 实现“仅在对象未变化时更新”。条件失败返回 412，应重新读状态再决定；并发删除还可能导致 409/404，分段上传冲突需要按 API 规则重新发起。不要无限重试旧条件，也不要把 ETag 一律当作内容 MD5。这里的条件操作不提供多个对象之间的事务。[AWS 条件写文档](https://docs.aws.amazon.com/us_en/AmazonS3/latest/userguide/conditional-writes.html)（2026-09-26）
...
托管产品的性能模型通常是"基线随容量线性增长 + 产品上限或扩容审批"。以阿里云官方规格为例：CPFS 通用版提供 100/200 MB/s/TiB 基线，常规公式的 IOPS 上限为 280 万；部分规格可申请提高到 1000 万 IOPS、100 GB/s。CPFS 智算版提供 400 MB/s/TiB 基线，公开上限为 3000 万 IOPS、2 TB/s，平均单路 4 KiB 读延迟参考值为 0.25 ms。上述均为规格或实验室口径，实际性能取决于容量、读写比例、并发与客户端配置，详见[通用版规格](https://help.aliyun.com/zh/cpfs/cpfsonecs/product-overview/product-specifications)和[智算版规格](https://help.aliyun.com/zh/cpfs/bmcpfs/product-overview/product-specifications)。买并行文件系统时应先确认：需要的是聚合吞吐还是单流吞吐？是数据 IO 还是元数据 IO？这两组答案决定规格档位，而不是"总容量"。
...
- **训练数据集管理**：数据集本身进对象存储做权威源（版本化、不可变快照），训练时经并行文件系统或缓存层供给；元数据（样本索引、标注版本）进 catalog/数据库。"数据集版本 = 对象存储前缀 + 清单文件"是一种便于审计与复现的做法。数据集的 IO 模式要分开治理：
...
- **向量数据的存储形态**：2025 年的对象存储开始原生吃向量。Amazon S3 Vectors 于 2025 年 7 月预览、12 月 GA（14 个区域、规模为预览期 40 倍），引入 vector bucket 与向量索引 API，把"存向量 + 近似检索"做成对象存储的一等能力，官方定位是显著降低向量存储与查询成本。工程判断：它不会取代专用向量数据库（延迟与混合查询能力仍有差距），但会吃掉"大规模、低频查询、成本敏感"的那一段——RAG 的离线索引、长尾知识库向量、Agent 的工具/记忆 embedding 池。向量检索在 RAG 中的角色见[RAG 架构](/ai/application/rag-architecture)。
...
- 一句话判断：**GPU 利用率上不去，先看存储带宽曲线，再看算力**。常见的"训练慢"投诉里，根因是 IO 的占了相当比例，而大多数人第一反应是查网络和卡。
...
另有 ESSD AutoPL 类"性能自动突发"档位，适合基线低但偶有尖峰的负载（突发部分按量计费），规格以官方块存储性能文档为准。两条常见工程建议：**多数事故源于"买了容量但没买够 IOPS"**——比如给数据库分了 100 GiB 的 PL1 盘，公式只给你 6,800 IOPS，离 50,000 上限远得很；**盘买对了还要看实例**——官方文档明说实例规格有最大存储 IOPS/带宽上限，云盘理论值 10 万 IOPS 挂到上限 6 万的实例就只有 6 万。选实例和选盘要一起算。
...
2. **冷数据下沉**：先跑访问分析（如 S3 Storage Lens / OSS 访问统计）确认真冷再下沉，常见把"季度才访问但每次全量读"的数据扔进深度归档，解冻费比存的还贵。
...
最常见的误区是：**只备份不演练，RTO 从未验证过**。备份成功的定义是"在目标 RTO 内恢复出可用系统"，而不是"快照任务绿色"。至少每季度做一次真实恢复：从跨地域快照开实例、从归档层解冻数据计时——你会对"懒加载首读慢""解冻要等几小时""跨账号权限没配好"这类问题有全新体感。
...
| 快照生命周期 | 删除后费用未按快照数同比减少 | EBS 保留仍被引用的块；按保留策略删除，并验证恢复点、AMI 关联及归档条件 |
