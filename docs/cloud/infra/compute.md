...
灵活性排序是官方结论：**节省计划 > 预留实例券 > 包年包月**。经验建议组合：稳态基线用包年包月或节省计划承诺掉（承诺消费类方案），弹性波动部分按量 + 伸缩组自动开关，可中断任务全部抢占式。单一模式很难最优，**混用比单模式省 30%+ 是多数稳态业务都适用的量级**；但承诺类方案的前提是负载真的稳——先攒 3 个月账单数据再做承诺决策，否则省下的折扣会被闲置承诺吃回去。
