...
![区块链时代技术编年总览](/images/chronicle/blockchain/blockchain-overview.png)

> 图：本页技术脉络总览。

> 这是[技术编年史](/chronicle/)的第三浪，也是**最适合用来研究技术泡沫与制度化**的一浪。适合所有正在为"要不要用某项新技术"做决策的人阅读：架构师、技术管理者、以及任何被"趋势"追着跑的工程同学。本文提供一条从 2008 白皮书到 2026 稳定币立法的全球编年、一条 2017–2019 中国联盟链主窗口的产业编年，以及账本、共识、智能合约和扩容原理。其可复用的判断框架是：**看一项技术是不是刚需，先问“不用它会怎样”**。本篇保持技术中立，只谈工程与架构，不涉任何投资判断；价格与市值数字仅作为编年锚点。
...
2013 年底，Vitalik Buterin 发表以太坊白皮书，把比特币的"专用脚本"改造成**通用状态机**：链不再只记账，还能执行图灵完备的程序——智能合约。2014 年以太坊用约 31,000 枚比特币的众筹完成冷启动（Wikipedia"Initial coin offering"词条），2015 年 7 月 30 日 Frontier 主网上线，"可编程的分布式信任"第一次成为可用的基础设施。智能合约由此成为独立技术方向：它的魅力在于**规则的自动执行不依赖任何一方的善意**——这正是后来所有 B 端"信任叙事"的技术原点。
...
以以太坊生态为主的数据表明，扩容战争到 2026 年已分出阶段性胜负——Rollup 路线赢了"单体链扩容"路线，L2 承接了绝大多数日常交易，L1 退居结算与担保层；但碎片化（几十条 L2 之间跨链桥接的体验与安全问题）成了新的主要矛盾。**这又是一次"解决一个问题、暴露下一个问题"的标准工程演进。**
...
把区块链的工程含义压缩为三句话，每句话背后都是一个真实的工程权衡。
...
- **供应链金融与溯源**：核心企业的应收账款凭证上链，多级供应商凭链上凭证融资——多方对账场景天然符合联盟链模型。工程上更常见的价值排序是：**存证 > 支付，多方记账比对 > 交易清结算**。

### 死掉的场景："为链而链"的 POC

数量远超想象。很多需求——单一主体主导的数据共享、可信赖第三方本来存在（政务中心、行业监管方）、性能要求高于每秒千笔——中心化数据库完全够用，却硬被套上链。识别方法可压缩成一个问题：**不用区块链技术会怎样？** 如果答案是"也没怎样，最多换个审计日志"，这个项目就注定在 POC 之后没有生产化。把这个识别方法画成决策图：
...
这个周期给技术人的三条提炼：其一，**每次崩盘都消灭一批玩家，但都被下一轮制度吸收**——Mt.Gox 催生日本交易所牌照制，FTX 催生美国立法议程；其二，**价格叙事与工程叙事是两条独立曲线**（2022 年金融信用破产与合并成功同岁）；其三，**"去中介"的终点很可能是"新中介受监管"**——ETF 托管行、持牌稳定币发行商、合规交易所，恰是当年白皮书想消灭的角色。这并非对初始理念的简单背叛，而是这是所有基础设施技术的宿命：先野蛮生长证明存在必要，再被制度收编获得大规模采用。
...
2024 年之后，公开案例中的海外云区块链负载出现了新的重点：交易所与做市商的云化基础设施、稳定币发行方的储备审计与链上监控、代币化平台的合规管线——**需求方从“联盟链 POC”转向持牌机构的工程部门**。这与元宇宙浪“付费的人换了一批，架构没换”是同一种代谢。
...
从云架构视角回看，可以得出一个分层结论：**区块链的技术命题是真问题，工程答卷有一半是真答案，商业叙事的大部分是泡沫**——而有趣的是，三者各自独立演化。真问题（多方互不信任如何协作）至今仍在，所以联盟链活着；真答案（共识工程、混合存证架构、Rollup 分层、账户抽象）被云与金融基础设施吸收，所以这浪"没白交学费"；泡沫的部分则由 Mt.Gox、LUNA、FTX 的受害者买了单。

给正在经历 AI 热浪的团队，可以留下三条经验：把 2017–2019 年当作"技术泡沫识别框架"的训练数据——
...
2. Ethereum.org. *Ethereum Whitepaper* / *Fork History* / *The Merge* / *Account Abstraction*. <https://ethereum.org/en/whitepaper/> ; <https://ethereum.org/en/ethereum-forks/> ; <https://ethereum.org/en/roadmap/merge/> ; <https://ethereum.org/en/roadmap/account-abstraction/>；Ethereum Foundation. [Ethereum Launches](https://blog.ethereum.org/2015/07/30/ethereum-launches)（Frontier 于 2015-07-30 启动的一手公告）
...
17. 美国国会. *S.1582 — GENIUS Act*（法案文本、审议与签署状态的一手记录）. <https://www.congress.gov/bill/119th-congress/senate-bill/1582>；背景交叉核对见 [GENIUS Act — Wikipedia](https://en.wikipedia.org/wiki/GENIUS_Act)
18. 美国证券交易委员会. [Statement on the Approval of Spot Bitcoin Exchange-Traded Products](https://www.sec.gov/newsroom/speeches-statements/gensler-statement-spot-bitcoin-011023)（2024-01-10，确认批准多只现货比特币 ETP 上市交易）

**编年事实来源（Wikipedia 条目，访问日期 2026-09-05）**

19. [History of bitcoin](https://en.wikipedia.org/wiki/History_of_bitcoin) — 交易所被盗编年（Mt.Gox 70% 份额、Bitfinex、NiceHash 等）、链上任意数据嵌入研究（2018）、FTX 破产（2022-11-11，约 80 亿美元缺口）。
...
> 注：本篇不构成任何投资建议；文中价格、市值、交易量等数字均为公开来源的编年锚点（来源见上），监管定性采用官方公开文件表述，不包含客户信息或未公开数据。
