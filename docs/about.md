...
reviewScope: 内容复核范围与日期规则、更新记录入口
---
...
“完整文章”描述组织形式，不代表全文所有事实均已重新核验。导读卡区分历史复核记录与专项复核；专项日期只覆盖列出的范围。页面修订日期来自代码修改，也不等同于事实核验日期。详细状态见[本页更新记录](#updates)。

## 知识支柱一览

- [云计算](/cloud/)：基座 / 计算·存储·网络 / 数据 / 云原生 / 架构治理
...
站点导航已将鹈鹕测试置于编年史之前，核验与更新记录统一收录于关于页。

本日新增 [鹈鹕测试](/playground/pelican/) 趣味展区：首批收录 13 个作品，支持按模型与 reasoning effort 筛选；总览动画默认播放，详情默认展示完整画面，可按需切换适应窗口。原始提示词与输出保留；这项展示不构成模型综合能力排名，也不更新其他文章的事实复核日期。
