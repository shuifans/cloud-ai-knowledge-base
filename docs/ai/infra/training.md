...
![显存账本、并行策略与训练迭代总览图](/images/ai/infra/training/training-overview.png)
...
这个算例给出三个读数：70B × 8K 全参训练在其精度、优化器、重计算和并行假设下需要多机多卡；选择性重计算的收益取决于线性项与注意力项占比；加入 TP 与 SP 后各项显存又会重新分配。**这些旋钮相互联动，不能把本例卡数当作所有 70B 训练的固定门票**，因此应先固定训练精度与优化器状态，再计算并行方案。

### 通信缓冲与框架开销

DDP 的梯度桶、ZeRO/FSDP 的临时分片、TP/PP 的 send/recv buffer，量级在百 MB 级，账本上不起眼，但**边界 OOM 的第一嫌疑人就是它**——"理论算得刚好、一跑就爆"几乎都是这块没留余量。工程估算建议：显存预算留 10–15% 余量给它，长序列场景留更多。
...
核心权衡是：**ZeRO 各阶段在显存、通信和实现复杂度之间取舍**。PyTorch FSDP 与 ZeRO-3 思想相关，但实现和语义不应简单画等号；Megatron Distributed Optimizer 主要切分优化器状态。Offload 可把状态或参数移到主机内存/NVMe，但性能影响取决于互连、重叠和访问模式，不能统一断言会慢固定倍数。
...
通信代价是 TP 的关键约束。Megatron 论文给出的通信次数和算量基于其具体分区方式，其他实现可能使用 reduce-scatter/all-gather、序列并行或异步重叠。TP 通常优先放在高带宽域内，但能否跨机取决于模型、消息大小、网络和实现，不能用固定卡数作通用边界。
...
监控应记录各专家 token 计数、负载方差与 drop 率。Capacity factor 的合理值取决于路由器、batch、并行规模和是否采用 dropless 实现；“1%–2%”不能作为跨模型统一告警线，应从收敛与吞吐实验中确定。预训练过程中调整该参数也必须验证是否改变训练分布。
...
| 默认 | 度数 ≤8 且头数整除时优先 | 超长序列、跨机、度数大时优先 |
...
两条一线纪律：**恢复演练**——checkpoint 只写不读是常态风险，定期（建议每两周）用真实作业做一次"从最近 checkpoint 恢复并跑通 N 步"的演练，顺带验证 reshard 路径；**间隔经济学**——checkpoint 间隔 = f(单次存盘开销, 平均故障间隔, 可接受回退步数)，万卡集群平均故障间隔以小时计，把间隔设成"一天一存"等于把有效训练时间白白送给故障。

**Loss 尖峰**的常见根因是数据批次污染（重复文档、编码损坏）、学习率与 warmup 不匹配、数值溢出三类。处理流程可固化成一张决策图而不是靠人肉值班：
...
**梯度累积**：`global batch = micro-batch × 累积步数 × DP 宽度`。它把有效批大小与单卡显存解耦，并可降低同步频率；代价不只有墙钟时间，还可能影响优化动态、数据顺序与批归一化语义。调整累积步数后应按真实 global batch 复核学习率、warmup 与收敛曲线。
...
优化顺序不应固定。应先通过 profiler 和端到端利用率分解确认瓶颈，再在通信计算重叠、流水线气泡、算子、数据管线与故障恢复之间排序。MFU 只描述有效算术利用，不能单独代表数据等待、重启损失和完整训练账单。
...
| m ≥ 10p | 气泡压到 10% 内的 micro-batch 经验下限 | 经验边界 |
...
| 1e-5 / 1e-4 | 全参 SFT / LoRA 的学习率量级 | 经验边界 |
| 0.1–2% | warmup 占总步数的经验区间 | 经验边界 |
