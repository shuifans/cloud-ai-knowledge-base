...
在本页算例假设下，**折旧占比高于电力**。该结论只适用于给定卡价、利用率、电价和折旧年限，且尚未包含服务器、机柜、网络与人力；不能外推为所有部署的固定成本结构。
...
同样的卡，利用率 100% 与 70% 时每个“有效卡时”的理论成本相差约 43%（有效成本 = 名义成本 ÷ 利用率）。利用率是成本敏感变量之一；它与卡价、电价、并发和 SLO 的相对影响应通过敏感性分析判断。连续批处理、前缀缓存与 PD 分离的重要经济目标，都是提高有效利用率。
...
倍数本身也有信息量：**轻量模型倍数普遍更高（8~10 倍），旗舰反而低（3~6 倍）**。可解释为，轻量档输入价被价格战打到接近白送（¥0.15/$0.10 每百万），输出价必须独自扛起毛利；而 DeepSeek 3 倍的"扁平"结构与其"全按成本定价、不赚档位差价"的市场策略一致。做预算时不能拿一个倍数套所有模型。
...
推理努力等级通常通过改变思考 token 数量影响账单，但不同厂商对隐藏思考 token、输出 token 与缓存的计费规则不同，必须以具体模型价目页为准。未来是否出现按推理档位差异化单价尚不能确认；Flex、Priority 与 Batch 是处理或调度模式，也不等同于推理努力等级。
...
历史数据表明部分能力档位的 token 单价快速下降，但“2026 年后斜率放缓”属于预测，不能写成已确认事实。单价下降也不等于总账单下降：调用量、上下文长度、推理 token 与 Agent 重试都可能抵消降价。观察价目表时应同时记录模型快照、服务档位、缓存、Batch、地区与采集日期。
...
按杠杆大小排序的买方优化手段（节省幅度为公开口径与工程量级估计，注明边界）：
...
执行顺序可优先从路由、上下文裁剪和缓存开始，再评估批量化、错峰、量化与引擎优化；具体优先级应由成本剖析决定。使用旗舰模型处理简单分类、每轮重复发送不变工具定义都是常见浪费，但节省比例必须按实际账单测量。
