...
reviewScope: KV 单位与容量复算、A100 精度口径、MoE 与 TP 适用边界
---

# GPU 选型与推理成本测算

![显存账本、卡型筛选与部署路线测算图](/images/ai/infra/inference/gpu-sizing/gpu-sizing-overview.png)

*本站生成的高清全文阅读地图；具体版本、参数与结论以正文引用的一手来源为准。*

> "我们要上大模型，需要买几张卡？"——这是容量规划中的高频问题。答案从来不是拍脑袋，而是一道可以算的应用题。这篇面向要做推理容量规划与硬件预算的架构师和工程师，把整条测算链一步步拆开：**显存四件套（权重 / KV Cache / 激活 / 框架开销）怎么逐项算、卡型谱系在 2026 年的真实格局、显存带宽为什么才是 decode 的真正瓶颈、量化与并发怎样改写卡数结论**，最后落到"按模型规模 × 并发 × SLO 三步定卡型定数量"的决策树，以及自建/租云/API 三条路线的成本比较方法。所有卡规格与报价均标注官方页或公开来源与访问日期（2026-09 刷新，Hopper/Blackwell/Blackwell Ultra 现役阵容）。全文主线一句话：**模型定显存、带宽定速度、并发定卡数、利用率定路线**。
...
负载瓶颈取决于输入输出比例、批大小、上下文长度、注意力结构及通信。小批量 Decode 常受显存带宽限制；长输入、大批量或专家通信也可能使计算与互联成为瓶颈。显存决定容量，实际吞吐应在相同模型、精度和 SLO 下比较，不能只凭带宽或峰值算力推断。
...
- **MoE 分开算存储与计算。** DeepSeek-V3 的全部权重仍需存储；纯 GPU 驻留时按总参数规划显存，采用 CPU/分层卸载时还要计入传输和延迟。激活参数可近似描述专家计算量，但不是包含注意力、路由和通信的完整成本。
- **权重刚好塞满 ≠ 能服务。** 70B FP16 = 140GB，H200 是 141GB：权重放下后 KV 与开销为零，实际难以留出足够运行空间。应按目标上下文与并发计算 KV 预算，不能把固定 20–30% 当成通用门槛。
...
```text
每 token KV = 2 × 80 × 8 × 128 × 2 = 327,680 字节 = 320 KiB
4K（4,096 token）单请求  = 1.25 GiB = 1.342 GB
32K（32,768 token）单请求 = 10 GiB = 10.737 GB
64 并发 @4K              = 80 GiB = 85.899 GB
八张标称 80GB 卡的容量需按实际可用字节相加，不能把 80GB 误作整机容量。
这里仅计 KV；还要加权重、激活、通信和框架开销。
...
| --- | --- | --- | --- | --- | --- |
| Llama-3-8B 级：32 层，GQA 8 KV 头 | 8 × 128 | 128 KiB | 0.5 GiB | 4 GiB | 32 GiB |
| Llama-3-70B 级：80 层，GQA 8 KV 头 | 8 × 128 | 320 KiB | 1.25 GiB | 10 GiB | 80 GiB |
| DeepSeek-V3 级：61 层，MLA（512+64 维缓存） | 等效 1 × 576 | 68.625 KiB | 0.268 GiB | 2.145 GiB | 17.156 GiB |
...
1. **KV 随上下文与并发的乘积增长。** 上述 70B 配置在 32K、64 并发下仅 KV 就为 640 GiB；不能把它与 140GB 权重直接按同单位比较。
...
工程估算可在公式合计后预留约 10%–20% 余量，但这不是固定标准；框架、CUDA Graph、通信缓冲区与碎片化都会改变开销。最终必须在目标引擎、批大小和序列长度下实测峰值显存，不能只依赖固定系数。
...
下表是硬件规格参考，不是现货清单。2026-09-26 重新核对了 A100 的精度口径；其余条目保留原 2026-09-05 快照，部署前按具体 SKU 核对。HTTP 可访问不证明在售，单卡、基板和整柜也不能混用。不同精度的稀疏/稠密数值须分别引用，不统一套用“一半”。

| 型号 | 定位 | 显存 | 显存带宽 | 代表性算力（口径） | 卡间互联 | 官方规格页 |
| --- | --- | --- | --- | --- | --- | --- |
| A100 80GB SXM | Ampere | 80GB HBM2e | 2,039 GB/s | FP16 312 TFLOPS 稠密 / 624 稀疏 | NVLink 600GB/s | [A100 官方规格](https://www.nvidia.com/en-us/data-center/a100/) |
...
| HGX B300（Blackwell Ultra） | 2026 旗舰 | 单卡 288GB HBM3e | 单卡 8TB/s | FP4 数值需按当前 HGX 对应 SKU 复核，不沿用原表冲突数值 | NVLink 1.8TB/s/卡 | [HGX](https://www.nvidia.com/en-us/data-center/hgx/) |
...
| PCIe Gen4/Gen5 | 32–64GB/s 单向 | 可运行 TP/PP，但需测量通信开销，不能沿用 NVLink 下的效率 |
| 无互联（消费级） | — | 无 NVLink 不等于无 PCIe 通信；需核对 P2P、驱动与引擎支持 |
...
工程建议：从满足显存需求的较小 TP 度数开始，在相同请求分布下测量 TTFT、ITL 与总吞吐。TP 分摊权重读取，但增加集合通信；PCIe 上也可以实现 TP，是否值得取决于拓扑、模型与 SLO，不能断言 Decode 阶段一定获益。

### 3.4 中国区合规卡与消费级卡的现实

采购可得性取决于目的地、最终用途、许可与实际供货，不能由显存或性能阈值直接推出“合规可买”。原先关于特定获批企业数量的二手描述已移除；采购时应按当前官方规则和供应商可交付条款确认。[BIS 官方更新](https://www.bis.gov/news-updates)（2026-09-26）

原引用路径未能确认有效，以下仅保留历史报道规格的两款（TechPowerUp 数据库与 SemiAnalysis 拆解，访问日期 2026-09-05）：
...
- **但有三个硬约束**：无 ECC（长时服务的静默错误风险）、无 NVLink（多卡只能 PP/单机多实例）、许可条款对数据中心部署不友好。建议：开发验证、边缘与小流量场景用消费级，生产底座不用。

国产加速器在选型表里的位置也要写实：多家产品已形成稳定供给，生态（算子覆盖度、主流推理框架适配、量化通路）仍在追赶，实际落地的工作量主要在适配与回归测试上，而不是硬件本身。典型用法包括两类：**合规与信创要求驱动的必选项**，以及**对单一供应商供给风险的对冲仓位**（用一小部分流量长期跑在第二生态上，保持热备）。评估时的关键动作是用自己的真实模型与算子清单做适配摸底，而不是看厂商对标表。
...
手算上限需要一个外部锚点来校验"量级没算错"。行业公开基准里，MLPerf Inference 的 datacenter 赛道从 v4.0（2024-03）起以 Llama 2 70B 的 tokens/s 为主负载，v5.0（2025-04）升级到 Llama 3.1 405B + 128K 上下文并给出 TTFT 6s / TPOT 40ms（P99 25 tok/s）量级的延迟门槛，v5.1（2025-09）又加入小模型负载。建议：**用手算上限对照同卡型同负载的公开提交值，两者在同一量级才放行方案**；差距超过 2–3 倍通常意味着口径错了（稀疏/稠密、offline/online、是否含量化）。
...
- **KV 量化能降低缓存字节数**，但有精度、缩放元数据与内核支持的代价，不能称为“免费并发翻倍”。
...
显存、计算、通信或排队都可能先达到上限；容量公式给出候选范围，延迟 SLO 与实测决定可用并发。

### 6.2 一个手算算例：70B FP8，2×H100，TPOT SLO 50ms

```text
假设可用显存 = 2 × 80GB × 0.9 = 144GB
假设权重与开销 = 76GB，则 KV 预算 = 68GB
4K FP16 KV = 1.34217728GB/请求
容量上限 = floor(68 / 1.34217728) = 50 请求

理想带宽下限（不计计算、通信与调度）
权重读取时间 = 70GB / 6700GB/s = 10.45ms
KV 读取时间 = B × 1.34217728GB / 6700GB/s ≈ B × 0.2003ms
理想下限满足 50ms 只是必要条件，不能证明真实延迟满足 SLO。
若实测 50 并发的平均步长为 50ms，则输出吞吐才可估为 1000 token/s。
```

FP8 KV 在不计额外开销时可将容量上限提升到约 101 请求，但量化质量、算力与通信可能成为新的限制，不能把“容量翻倍”写成“吞吐翻倍”。以上所有余量与吞吐均是带条件的预算例子，需要实测校准。
...
并发-显存-延迟三角的每条边都标着价格：提并发要 KV 显存（加卡或量化 KV），压延迟要带宽（升代际或加 TP），而两者共同决定单 token 成本。工程排序应是**先满足 SLO（不可谈判），再在剩余空间里最大化并发（摊薄成本）**，而不是反过来先压成本再谈体验——后者在对话类产品上几乎必然返工。把 6.2 的三个数（KV 预算、SLO 反推并发、聚合吞吐）与第七节的卡时报价放进同一张表，"每百万 token 成本"就会随并发取值呈现一个明显的 U 型：并发太低摊不薄权重读取，并发太高撞 SLO 后要加卡。U 型底部附近 ±20% 就是合理的运行点。
...
实测口径有三个必须写进报告的字段，否则数字不可比：**并发档位与调度策略**、**输入/输出长度分布**和 **SLO 达标率**。只报告 offline 聚合吞吐会掩盖尾延迟，因此聚合吞吐必须与 P99 延迟成对出现。

### 7.4 弹性与潮汐：固定底座 + 弹性层

峰谷比大的负载按峰值配固定底座会大量闲置。默认结构：
