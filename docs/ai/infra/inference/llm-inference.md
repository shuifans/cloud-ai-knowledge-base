...
reviewScope: KV 单位、分页容量及 PD 分离传输算例
---
...
代入 70B 级 GQA 配置（80 层、8 KV 头、头维 128、FP16）：每 token KV 为 327,680 B = 320 KiB。128K（131,072 token）请求的 KV 为 40 GiB，约 42.95GB。该数值是整模型缓存，分摊到各卡还取决于并行实现；不应直接当作单卡占用。
...
分页算例：上述配置每 16 token 的 KV 为 5 MiB。如果整个副本可用于 KV 的总预算为 40 GiB，则可容纳 8,192 块；每请求 2,048 token 占 128 块，容量上限为 64 请求。实际要扣除尾块、共享、预留与分配开销，并通过延迟 SLO 验证；不能据此承诺相对其他分配方式必定翻倍。
...
goodput 是 DistServe 强调的重要指标：高吞吐若以请求延迟同时超出 SLO 为代价，对用户并无有效价值。因此容量核定与压测验收应同时报告 goodput、throughput 与明确的 SLO 定义。
...
DistServe 论文把这个矛盾讲得很透：混跑会把两个阶段的资源分配与并行策略耦合在一起，在严格的延迟要求下，系统要么牺牲其中一端的延迟，要么为两端都过度配置算力。典型症状是：**开了 chunked prefill 之后，长上下文流量一来，TPOT 的 P99 仍然锯齿状抖动**——这时就该认真考虑分离了。
...
KV 传输算例：上述配置 32K 缓存为 10 GiB，即 10.737GB。假设实际有效网络吞吐为 40–50GB/s，纯数据搬运约 215–268ms；还要考虑连接、排队、并行分片和流水重叠。对 TTFT 的实际增量不一定等于全量串行搬运时间，应在相同请求分布下比较分离前后的端到端表现。
...
实践中应注意四点：仅在延迟敏感场景评估；用真实流量验证接受率与端到端 TPOT；把草稿模型或草稿头占用纳入显存核算；不要默认在吞吐型服务开启投机解码。厂商基准不能替代目标模型和流量分布下的实测。
...
- **不知道选什么就选 vLLM**。模型支持与社区生态最广，踩过的坑都有人趟过，可将其作为评测基线
...
选型之后是调参。下面这张表可作为部署方案检查单（参数名以引擎当期版本文档为准，含义跨引擎通用）：
...
| --- | --- | --- | --- |
| 70B 级稠密 GQA（80 层 / 8 KV 头 / 头维 128 / FP16） | 320 KiB | 10 GiB | 40 GiB |
...
"设计一个能扛 10 倍突发流量的推理服务"是面试与方案评审的高频题，可按三层展开：
