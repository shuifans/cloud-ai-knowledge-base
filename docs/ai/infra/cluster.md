...
![GPU、节点、机架与数据中心分层拓扑图](/images/ai/infra/cluster/gpu-cluster-overview.png)
...
- 选型与常见坑——规模 → 形态 → 拓扑 → 存储的决策顺序，以及常见问题。
...
**架构含义**：并行策略要贴合硬件拓扑。通信频繁且处于关键路径的 TP/EP 通常优先放在高带宽域内，PP 与 DP 再按可重叠程度向外扩展；实际映射需用通信剖析验证，不能把单一拓扑规则视为所有模型的固定答案。
...
**做容量规划时应按"域"而不是按"卡"做单位**：买多少卡不重要，买多少个 NVLink 域、域间网络怎么接，才决定并行策略的可行空间。一个直观的账：405B 级模型 TP=8 放在 HGX 域内刚好；若域只有 8 卡而模型需要 TP=16，要么换 NVL72 级机架，要么把 TP 拆成"域内 TP8 + 域间 TP2"并接受跨域 AllReduce 的代价——后者在多数场景不划算。
...
这张图说明：**RoCE 的便宜是拿网络工程团队的水平换的**。PFC 死锁、ECN 阈值、打流不均、光模块一致性，任何一项没管住，训练 MFU 就会以百分点为单位往下掉。IB 则把这部分复杂度收进了子网管理器（配套 UFM 类工具），代价是生态锁定与采购来源单一。
...
收敛比是胖树设计的重要成本变量，但并非唯一变量。无阻塞网络与收敛网络的取舍应基于目标并行策略、流量矩阵、故障域与扩容方式；只有在通信模型和压测证明相应流量可重叠或可容忍时，才适合引入收敛。
...
拓扑决定逻辑，物理层决定能不能落地。800G 一代常见问题，集中在这几件事：
...
云上 RDMA 的调优边界更多由平台控制。EFA、eRDMA 等方案能否满足训练需求，取决于实例拓扑、集合通信库、模型通信比例和规模；“千卡以内够用”不是可跨云、跨模型复用的结论，必须通过目标规模压测确认。
...
- **In-Network 归约**：InfiniBand SHARP 可把部分归约操作下沉到网络。实际收益与交换机代际、消息大小、集合通信实现和作业并发有关；是否启用还需评估故障诊断、兼容性与运维复杂度。
...
工程上应注意：**检测与隔离的自动化程度，比备机比例更决定有效训练时长**。备机再多，靠人肉判断换机，万卡集群也跑不出好看的 MFU。
...
- 备机比例不应套用固定的 3%–8% 经验区间；应根据节点故障率、备件补充时间、作业恢复机制与目标有效训练时长反推。备机还应尽量与主力配置一致，避免准入基线偏差。
