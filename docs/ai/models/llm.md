...
reviewScope: KV 与 MoE 容量算例、当前 OpenAI/Claude/Qwen/DeepSeek API 目录；其他历史参数未全量重核
---

# 大语言模型架构解析

![Transformer 骨架、训练阶段与架构演进总览图](/images/ai/models/llm/llm-overview.png)
...
> 目标读者是要做模型选型或推理方案设计的架构师与工程师；读完应能把"模型卡片上的架构特性"翻译成"部署要花多少钱、能承载多少并发、思考预算怎么配"。
...
以下容量统一使用二进制单位：1 KiB = 1,024 B，1 GiB = 2³⁰ B；128K 表示 131,072 token。仅计算缓存，不含权重和运行时。

| 配置 | 单 token KV | 128K 单请求 |
| --- | ---: | ---: |
| MHA：80 层 × 64 KV 头 × 128 维，FP16 | 2.5 MiB | 320 GiB |
| GQA：80 层 × 8 KV 头 × 128 维，FP16 | 320 KiB | 40 GiB（42.95 GB） |
| GQA：32 层 × 8 KV 头 × 128 维，FP16 | 128 KiB | 16 GiB |
| MLA：61 层 ×（512+64）缓存维度，BF16 | 68.625 KiB | 8.578 GiB |

70B FP16 权重约 140GB，10 路上述 128K GQA 请求的 KV 为 400GiB，已经超过权重。8B 模型在一张卡上的并发上限应先按实际可用字节扣除权重、激活和运行时，再除以每请求 KV；不能把标称 80GB 整块显存都分给缓存。

FP8 KV 的元素字节数可减半，但量化缩放、页对齐和质量影响仍要检查。前缀共享可减少重复占用，滑动窗口与混合注意力也会改变公式。容量验收需覆盖峰值上下文与并发，而非只代入平均单轮长度。[更完整的容量算例](/ai/infra/inference/gpu-sizing#_2-3-kv-cache-并发与上下文的放大器)
...
负载均衡方案是评估 MoE 实现可复现性的一个观察点。论文若公开路由、辅助损失或偏置更新机制，复现条件通常更清晰；但仅凭披露方式不能判断训练工程成熟度，还需结合代码、训练日志与独立复现。
...
2. 单节点 8 × 80GB = 640GB，双节点 16 卡也只有 1,280GB，仍小于 1,342GB 权重。纯 GPU 驻留按标称值至少需要 17 张，实际还受整机卡数、并行度与余量约束，不能把“16 卡”作为可运行结论。
3. 换 FP8 权重约 671GB，仍大于 8 × 80GB 的 640GB，且尚未包含量化元数据、KV 与运行时。单节点部署需要更大显存、进一步量化或卸载，并重新评估 SLO。

**算例二：Kimi K2 Thinking 的 INT4 发布形态（不要泛指所有 K2 检查点）**
...
- **单价模型被改写**：传统“输入价/输出价”两元结构之外，多了思考长度这一变量。不同模型、任务和推理档位的隐藏/可见思考 token 差异很大，不能套用固定倍数；容量规划应基于目标模型的计费规则和真实任务分布，按“思考预算”而不是只按答案长度测算
...
**模型谱系（包含开放权重与托管 API，历史规格保留原资料口径）**：

| 机构 | 代表（发布时间） | 架构与规模要点 |
| --- | --- | --- |
| DeepSeek | 开放模型论文与当前 API 路由分别核对 | 当前 API 的 `deepseek-flash` 为 V4.1-Flash，旧 V4 Flash 名称会重定向；Pro 为 V4-Pro-0813。不能把 API 当前版本与旧检查点的架构参数混为一谈。[官方模型与路由](https://api-docs.deepseek.com/quick_start/pricing/) |
| 阿里 Qwen | Qwen3.5 开放模型与 Qwen3.8-Max 托管 API 需分开看 | 2026-09-26 官方页记录 `qwen3.8-max` 快照 `qwen3.8-max-0902`，支持图文与视频输入、文本输出；API 上架不能作为 Max 权重开放的证据。[百炼模型说明](https://help.aliyun.com/en/model-studio/qwen3-8-max) |
...
| 厂商 | 节奏（发布时间） | 公开要点 |
| --- | --- | --- |
| OpenAI | 当前 API 型号包括 GPT-6 Astra / Sol / Luna | 2026-09-26 价目已更新；具体端点、输入模态与推理档位按模型卡核对，不能把 ChatGPT 产品名称直接当作 API ID。[API 价格](https://developers.openai.com/api/docs/pricing) |
| Anthropic | Fable 5.1、Opus 5.5、Sonnet 5、Haiku 4.5 | 2026-09-26 当前模型目录包含 Opus 5.5；旧 Opus 5 仍应按独立版本处理。[模型目录](https://platform.claude.com/docs/en/models/overview) |
...
2. **注意力结构多样化**：稀疏、线性与混合注意力都在发展；闭源模型未披露的架构不能从定价或上下文长度反推。
3. **长上下文需要任务验证**：名义窗口、可用输出长度、长文召回和长请求费用是四个不同指标，不能用窗口长度代表质量。
...
**如何比较开放权重与闭源 API**：分别看同任务质量、长尾可靠性、工具调用、许可和运行成本。没有统一任务集与复现配置时，不给出“领先缩短到几个月”的总体数字；权重可下载也不代表所有训练数据与流程开放。
...
读卡三步法（建议流程）：**第一步只看总参与精度**，判断装不装得下、要几张卡；**第二步看注意力结构与 KV 口径**，用本文算例公式算并发；**第三步才看基准分**，且只信与自己业务同构的基准。顺序不能反——先被基准分种草再去凑硬件，预算通常会失控。
...
- **价格需使用相同口径**：模型、地区、档位、缓存、长度和优惠条件都会改变账单。当前快照见 [Token 经济学](/ai/infra/inference/token-economics)，不要用旧版本报价推断当前能力或每任务成本。
...
- [DeepSeek-R1（Hugging Face）](https://huggingface.co/deepseek-ai/DeepSeek-R1) — 官方权重、模型架构、使用说明与 MIT 许可证；部署时应固定 revision，而不是只记录可变的 `main`
- [DeepSeek-V3（Hugging Face）](https://huggingface.co/deepseek-ai/DeepSeek-V3) — 官方基础模型权重与模型卡；API 产品和开放权重 checkpoint 不应视为完全相同的运行实体
- [Qwen3-235B-A22B（Hugging Face）](https://huggingface.co/Qwen/Qwen3-235B-A22B) — Qwen3 MoE 官方模型卡、权重与 Apache-2.0 许可证
- [Llama 3.3 70B Instruct（Hugging Face）](https://huggingface.co/meta-llama/Llama-3.3-70B-Instruct) — Meta 官方模型卡；权重受 Llama 3.3 Community License 约束，不等同于 Apache/MIT 开源软件
- [Mixtral-8x7B Instruct v0.1（Hugging Face）](https://huggingface.co/mistralai/Mixtral-8x7B-Instruct-v0.1) — Mistral AI 官方 MoE 权重与 Apache-2.0 许可证

> 模型卡会更新。生产部署应同时记录仓库、精确 `model_id`、revision/commit、量化格式与许可证版本；API 别名、API 快照和开放权重 checkpoint 分开核验。
