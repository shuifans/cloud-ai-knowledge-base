...
reviewScope: 移除扩散多样性保证、唯一工业路线与可控性排他结论
---
...
2021 年之前图像生成的主流路线之一是 GAN（生成对抗网络：生成器与判别器进行对抗训练）。其工程难点主要集中在两方面：
...
2. **多样性需要评测**：扩散模型的训练目标提供了一种学习数据分布的方法，但不能保证有限数据、有限容量和有限采样步数下覆盖所有模式。
3. **过程每一步都可注入条件**：这个特性后来长出了整个可控生成生态（ControlNet、局部重绘）。

扩散与流匹配是重要的图像生成路线；GAN、自回归和混合路线也有工程应用，不能称某条路线为“唯一能工业化”。质量、多样性、延迟和可控性应在具体模型与任务上比较。

代价同样清楚：迭代采样慢（DDPM 原始设定约千步）。此后整条加速技术线——DDIM、蒸馏、一致性模型、缓存复用——都在为这个代价还债，直到今天。

## DDPM 拆开看：加噪、去噪与"预测噪声"

这一节是全篇地基，重点解释三件事讲透：前向过程为什么可以一步到位、反向过程网络到底在回归什么、以及 ε 与 x₀ 为什么是同一枚硬币的两面。
...
工程常用口径（经验值，跨模型不可照搬，任何模型都先从官方默认值起步）：
...
以 SD 1.x 的 8 倍空间压缩为例，512×512×3 的图变成 64×64×4 的潜变量，空间位置数减少 64 倍。实际计算量还取决于通道数、网络结构和注意力层，不能只由空间压缩比直接推出固定倍数；潜空间计算是其降低训练与推理成本的重要原因之一。
...
三件套定型之后，文生图的完整过程就可以画全了。无论是 SD 谱系、FLUX 还是 Qwen-Image，推理管线的骨架基本一致。方案说明可先使用这张图：有了它，生产调优问题至少一半能定位到具体环节——是编码没编好，还是步数不够，还是解码丢细节，一目了然。
...
2. **与 LLM 部分共享工程栈**：分布式并行（张量并行/序列并行/FSDP）、混合精度、量化与部分推理服务组件可复用；但扩散/流模型的调度器、VAE、图像预后处理和显存曲线仍有专门要求，不能假定整套 LLM 服务栈可直接复用；
...
与架构换代同步的是训练范式：**流匹配（Flow Matching）及 rectified flow** 直接学习概率路径的速度场。SD3 与 FLUX 等模型采用了相关路线，但不能据此断言所有新模型都以流匹配为默认；不同论文对路径、目标函数和采样器的定义也不完全相同。工程上应依据具体模型卡和实现确认训练与采样方式。
...
把这条时间线捋清楚，选型时就不会被新名词绕晕。下文只列工程上产生显著影响的节点。
...
- **1.5 的价值不在质量在生态**。它是最早"人人可微调"的一代，插件、LoRA、ControlNet 权重存量巨大。今天仍有团队用它跑垂直场景——不是因为效果好，而是因为"所需 LoRA 只有这一代可用"。技术债的典型形态；
...
“中文场景可用性超过同代 FLUX”缺少统一独立评测口径，不能作为普遍结论。中文物料、图内文字、许可与本地部署是重要选型维度；风格化质量、LoRA/ControlNet 生态也应在相同提示词、分辨率、采样配置和人工评分规则下比较。
...
下表仅列代表性方案，时间是首次发布时间；具体能力与价格以各方最新官方文档为准。
...
原生多模态图像生成受到关注后，出现了“自回归多模态将取代扩散”的讨论。公开产品与权重显示两类路线仍在并存，更准确的描述是**按产品目标分工**：
...
许可直接影响商用交付是否成立：**效果与许可必须同时核验**，技术验证通过不代表满足模型权重、托管服务、衍生模型与输出内容的使用条款。

## 可控性生态：冻结主干、旁路挂条件

只有文本条件，满足不了生产需求。真实业务要的是"人物姿势、身份、品牌风格与局部编辑都必须可控"。开源社区沿"**冻结主干、旁路挂条件**"的思路把可控生成做成了插件市场——这是开源生态相对闭源 API 最深的护城河：闭源侧只能开放参数，开源侧能长出新结构。
...
同期还有更轻量的 **T2I-Adapter**：不复制编码器半边，只增加小型适配器。ControlNet 与 T2I-Adapter 在参数量、控制强度和可组合性上各有权衡，不能脱离底模、条件类型与评测指标给出绝对优先级。
...
掩码式 inpainting 与指令式编辑适合不同控制粒度：前者便于指定像素区域并控制局部重算，后者交互更自然但可能改动非目标区域。无法以可靠证据支持“90% 的需求”这一比例；产品选型应比较细节保持、多轮一致性、局部可控性和全图重算成本。
...
公开榜单无法直接回答“模型在特定业务品类上是否好用”，因此需要建立业务评测集。可采用以下做法：
...
LCM 论文报告的蒸馏成本远低于从零预训练基座，但不同模型、数据和硬件口径不能直接用数量级概括。对大多数团队，基于合规的开放权重做 LoRA、ControlNet 或蒸馏通常比从零训练通用基座更可行；是否如此仍取决于数据主权、许可、目标能力和总拥有成本。
...
3. **负向提示的历史地位**：SD 1.5/XL 时代有效（`lowres, bad anatomy, extra fingers` 一类），DiT 时代（SD3/FLUX/Qwen-Image）官方口径已建议直接正向描述。建议保留一份最小负向词表（违规内容、明显畸变），不再堆几十行；
...
6. **提示词重写（prompt rewrite）**：DALL·E 3 公开过用 LLM 扩写用户提示的做法。自建管线可把重写做成可选开关，并在模板化输入和开放输入上分别评测遵循度、延迟、费用与用户意图偏移。
...
- **ComfyUI（GPL-3.0）** 是本地部署的事实标准：节点式工作流把"模型 + 条件 + 后处理"做成可自由拼装、可版本化、可 API 调用的工件，本地交互与云端批处理通吃。建议把工作流导出为 JSON 存进代码仓库，与提示词模板一起当配置管理；
...
生成质量依赖模型，交付稳定性还依赖管线。生产方案通常需要补齐以下能力：
...
- **与视觉理解的融合（生成—评估闭环）**：生成结果回灌多模态理解模型做质检与打分，形成自动化重试；其收益取决于裁判模型校准度、失败类型与重试预算，需避免自动评分偏差被闭环放大；
...
- [Stable Diffusion 3.5 Large（Hugging Face）](https://huggingface.co/stabilityai/stable-diffusion-3.5-large) — Stability AI 官方模型卡与 Community License；“可下载权重”不自动等于无限制商用
- [FLUX.1-dev（Hugging Face）](https://huggingface.co/black-forest-labs/FLUX.1-dev) — Black Forest Labs 官方模型卡与非商业许可入口
- [FLUX.1-schnell（Hugging Face）](https://huggingface.co/black-forest-labs/FLUX.1-schnell) — Apache-2.0 权重，与 `dev` 版本的许可和采样配置不同
- [Qwen-Image（Hugging Face）](https://huggingface.co/Qwen/Qwen-Image) — Qwen 官方模型卡、权重、推理示例与许可证

> 同一系列不同 checkpoint 的许可证可能不同。选型表中的“开放权重”只描述权重可获得性，商用、托管服务、衍生模型与生成内容责任仍应逐项阅读对应模型卡和许可证原文。
