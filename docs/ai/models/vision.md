...
reviewScope: ViT 归纳偏置、视觉 token 路线和训练成本边界
---

# 视觉理解：从 CLIP 到原生多模态

![视觉理解演进、VLM 接入方式与视觉任务总览图](/images/ai/models/vision/vision-overview.png)
...
这一阶段的主要瓶颈之一是标注成本。缺陷检测、证件识别和车牌识别等闭集任务通常依赖场景数据，类别或成像条件变化后往往需要重新采集、标注和训练。因此，模型精度之外还必须评估每个新场景的数据边际成本。
...
2. **网格切块**：按 P×P（16 或 14）切出不重叠的 patch，数量 N = H/P × W/P。224÷16=14，即 14×14=196 个 patch。patch 化只是其结构设计之一；位置编码、层结构与训练目标也引入偏置，不能称为“完全没有归纳偏置”。
...
6. **取输出**：分类任务取 [class] 位置输出过 MLP head；**VLM 可能使用 patch token，再经 pooling、resampler 或 projector 压缩/映射**——每个 patch token 就是后面要进 LLM 的"视觉词"。
...
这也是 CLIP、SAM 和大量 VLM 采用 ViT 系视觉编码器的重要背景。不过 CNN 并未消失，在端侧、低延迟、固定视觉任务和特殊传感器场景仍有价值；骨干选型应从数据、延迟、算力与预训练权重可得性出发，而不是按年份作绝对判断。
...
4. **规模**：CLIP 论文使用约 4 亿图文对。不同骨干的训练配置与计算预算不同，不能把单一训练时长概括为整套方法的成本，也不能由此断言比人工标注便宜固定倍数。
...
在 VLM 普及之前，CLIP 在工业界最常见的角色不是分类器而是**检索器与过滤器**：图像与文本同空间意味着"以文搜图""以图搜图"就是一次余弦相似度 top-k；数据管线里用它做图文相关性打分、近重复聚类、NSFW/品牌 logo 粗筛。工程实践中，给训练集做"图文配对质量清洗"用的就是这个打分——比人工抽检便宜两个数量级。这个用途今天仍然有效：VLM 太贵，不适合对百万级图库做全量扫描，CLIP/SigLIP 级别的轻量对齐模型仍是第一道筛子。
...
也要说清边界：CLIP 式对齐在**计数、空间关系、细粒度属性和 OCR**上存在已知限制，这正是后续 VLM 重点补齐的能力。CLIP 相似度不适合直接作为“图里是否有三根导线”这类计数任务的可靠判据；即使改用 VLM，也应通过结构化输出与业务标注集验证。

## VLM：把视觉接进 LLM 的三种接入方式

"把视觉接进 LLM"的范式在 2022–2023 年定型。所有方案本质上都在回答三个问题：**视觉 token 从哪来、进 LLM 的哪个位置、占不占上下文**。按答案分成三种接入方式，本文按"视觉信息注入 LLM 的位置"来分：
...
压缩桥的极端形态是 **BLIP-2**（Salesforce，2023-01）：冻结图像编码器与 LLM，中间训一个轻量 **Q-Former**（带 32 个可学习 query 的小 Transformer，约 188M 可训练参数），query 先自注意力、再与图像特征交叉注意力，最终把整张图压成 32 个向量 token 喂给 LLM——用极少的可训练参数接通两个冻结大模型。与 Flamingo 的区别在于：这 32 个 token 是**进入上下文序列**的，只是压缩率极高。压缩必然损失细节：通用图文问答够用，文档小字、密集表格这类"细节即答案"的场景会吃亏——因此文档细节任务不宜默认采用纯 Q-Former 方案。
...
选型时，MLP 投影类架构通常更易于私有化微调与行业适配；长视频、多轮多图等视觉 token 压力较大的任务可评估查询压缩类桥接结构。原生多模态架构的训练配方和权重公开程度差异较大，不能只按架构名称判断可部署性。
...
- **原生动态分辨率（Naive Dynamic Resolution，Qwen2-VL 起）**：不再固定缩放或固定网格，而是按原图宽高比直接分配视觉 token 数——每个视觉 token 对应约 28×28 像素（patch 14 + 2×2 合并），用 min_pixels / max_pixels 两个参数给 token 数设上下限。文档场景选型，首先应验证这一点：小字能不能看清，取决于给它的 token 够不够。
...
架构趋势上，视觉理解与视觉生成的边界正在模糊，但“增加像素输出能力”并不等于无需独立生成训练。组合式架构具备模块可替换、成本可控和便于私有化微调的优势；原生式可能提高交互一致性。选型应依据业务指标与可验证部署条件，而不是“架构先进”标签。
...
**意义**：视觉分割进入“预训练 + 提示”范式，降低了部分场景逐任务训练的门槛。常见应用包括数据标注辅助、图像编辑管线和机器人感知；但专业领域、精细边界和开放词汇分割仍需数据适配与专项评测，不能据此认为分割研究已被完全“收编”。

SAM 系与 VLM grounding 可以按输出目标分工：**像素级掩码优先评估 SAM，语义级区域优先评估 VLM grounding**。对于需要先理解语义再定位的目标，可先由 VLM 输出区域，再由 SAM 细化掩码；具体效果需在目标类别与界面上验证。
...
但工程结论是：**VLM 吃掉了理解层，没有吃掉识别层**。复杂版式的四个老大难仍在：
...
因此最稳的组合仍是：**专用 OCR 打底出"可信文本层"，VLM 做理解与抽取层**；关键字段（金额、证件号、日期）用 OCR 结果与 VLM 结果交叉校验，不一致即转人工。这个组合在工程实践中比"单模型硬扛"的返工率低一个量级。
...
是否可以取消 OCR 打底，必须由字段级准确率、人工回流成本和文档分布决定，不能套用固定的 1% 阈值。涉及复杂表格、遮挡或法律/财务级准确率要求时，应比较 OCR、VLM 与双管线交叉校验的错误类型和总成本。
...
默认做法：接口层统一收敛到"归一化 0–999 + 中心点"一种口径，模型原生输出在适配层转换；上线前用 20–50 张带人工标注的自家截图跑标定，统计系统性偏移（整体平移/缩放错误一眼就能看出来）。

GUI Agent 的基本循环是“截图 → VLM 识别元素并给坐标 → 执行动作 → 再截图”。OSWorld、UI-TARS 等公开结果依赖模型版本、步数预算与评测环境，数字必须附快照日期，不能跨版本直接相减。生产系统仍需回退与人工接管；grounding 能力必须使用目标界面截图实测，因为自绘控件、深色主题和中英混排可能偏离公开基准分布。
...
2. **两级策略控制成本**：先低分辨率粗扫定位事件区间，再对区间抽高密度帧精读；节省比例取决于事件稀疏度、抽帧率和模型计费方式，应通过目标视频集测量。
...
视觉输入能否复用取决于模型架构与供应商缓存语义，不能一概认为视觉编码必然与问题无关。常见的三种复用形态包括：
...
- [Qwen2.5-VL-7B-Instruct（Hugging Face）](https://huggingface.co/Qwen/Qwen2.5-VL-7B-Instruct) — Qwen 官方视觉语言模型卡、权重、输入格式与许可证
- [InternVL3-8B（Hugging Face）](https://huggingface.co/OpenGVLab/InternVL3-8B) — OpenGVLab 官方模型卡与部署示例
- [SAM 2.1 Hiera Large（Hugging Face）](https://huggingface.co/facebook/sam2.1-hiera-large) — Meta 官方分割模型权重；视频预测器与图像预测器的输入约束应结合官方仓库核验
- [SigLIP 2 So400m Patch14-384（Hugging Face）](https://huggingface.co/google/siglip2-so400m-patch14-384) — Google 官方视觉—文本编码器模型卡

> “支持图像/视频输入”不等于在计数、OCR、空间关系或长视频定位上可靠。模型卡只能确认身份、接口和厂商披露结果，业务准确率仍需在目标数据上评测。
