...
reviewScope: CNN 等变性与逻辑回归系数解释边界
---

# 机器学习与深度学习经典架构

![机器学习、神经网络、视觉、序列与生成模型谱系图](/images/ai/models/ml-dl/ml-dl-overview.png)
...
整个架构史可以概括成一句话：**每一代架构都把人对数据的某种先验假设编进结构里，而下一代用更弱的假设替换它，代价是换来更多的数据与算力需求**。线性模型假设"特征线性可加"，SVM 假设"存在一个最大间隔的划分面"，CNN 使用局部连接与权重共享，具有一定平移等变性；整体平移不变性还受池化、边界与模型结构影响，RNN 假设"历史可压缩为一个状态向量"，Transformer 则把结构假设降到最低——所以它必须靠最大的数据和算力喂养，回报是跨任务的普适性。
...
注意箭头方向：这不是“替代”，而是**分工的固化**。许多经典架构并未整体退场，而是作为组件继续存在，让更合适的结构接管主链路。理解这一点，后面每一节的“今天的位置”才有落点。

## 传统机器学习：特征工程时代

### 线性模型：低矮但地基扎实

逻辑回归（一句话解释：把特征的加权和过一个 sigmoid 映射成概率）是工业界最老也最耐用的模型。它的全部能力就是"特征线性可加"这一条假设，换来三个至今稀缺的性质：**训练成本可忽略、单条预测毫秒级、每个特征的权重直接可读**。风控审批要能向监管解释"为什么拒贷"，系数可帮助解释给定特征与模型条件下的关联，但受缩放、共线性和交互项影响，不能直接视为因果解释。
...
今天它依然存在，常作为复杂系统的兜底或校准层：在大模型或 GBDT 输出分数后，用逻辑回归等方法做概率校准。上线前应检查特征单调性/分箱、系数符号与业务先验，以及校准曲线在分数两端的失真。
...
SVM 的核方法在样本规模扩大时训练成本上升，多分类与概率输出也需要额外设计；但不能笼统断言性能上限与数据量无关。它仍适用于部分小样本、高维稀疏任务。样本规模较大且需要表征学习时，应把线性模型、树模型与深度模型纳入同口径比较，而不是按单一阈值排除 SVM。
...
| 典型适用场景 | 通用默认、需要稳 | 大数据量、追求训练速度 | 类别特征占比高的风控与广告数据 |

三个共通的超参经验（适用边界：万到亿级样本的表格任务）：**学习率与树轮数是一对跷跷板**（η 调小就把轮数加上去，通常 η=0.01~0.1 起步）；**max_depth/num_leaves 控制单棵树的表达力**，过深最先出现的症状是训练 AUC 涨、验证 AUC 平；**subsample/colsample 类随机化是免费的正则**，噪声大的数据上收益明显。调参顺序可设为：学习率 + 轮数 → 树深/叶数 → 采样比例 → 正则项。

### 为什么风控与表格场景至今仍是树模型的天下

需要避免绝对化判断：**在结构化表格数据上，深度模型没有稳定优势，而劣势是全方位的**。
...
风控、推荐 CTR 预估和搜索排序常包含大规模异质表格特征，GBDT 是重要基线；当输入转为图像、文本或行为序列等高维稠密信号时，深度模型更具优势。纯表格场景也不能只凭数据形态下结论，应以时间切分和业务指标比较树模型、深度模型与表格基础模型。

表格基础模型开始挑战“纯表格默认用树”的结论。TabPFN v2 的 Nature 论文报告其在指定小型表格任务和时间预算下优于若干调参基线；这属于论文评测口径，不能外推到所有大表或生产负载。它适合纳入小数据、快速原型的交叉验证，同时仍需比较 GBDT 的类别特征处理、可解释性与服务稳定性。商用前必须核验所用 checkpoint 的精确许可证，不能按系列名称概括。

出现以下信号时，应重新评估“表格是否还该用树”这个默认答案：
...
AlexNet 的意义不只在网络本身，也在于集中展示了大规模标注数据、GPU 训练、ReLU、Dropout 和较深卷积网络的组合效果。论文使用双 GPU 训练并在 ImageNet 竞赛中显著降低错误率，推动了深度学习在计算机视觉中的采用。
...
ResNet 论文解决的问题常被误说成“梯度消失”，但其关键观察是：**56 层普通网络的训练误差高于 20 层网络**。这不是典型过拟合，而是随深度增加出现的优化退化问题；残差连接让恒等映射更容易学习。
...
其作用可从三层理解：**其一**，解空间里永远包含"恒等映射"这个不差于浅网络的解（令 F(x)=0 即可），深网络至少不会更坏；**其二**，反向传播时梯度可以沿恒等捷径无损直达任意浅层，梯度消失被结构性绕开；**其三**，集成视角——残差网络行为上近似大量浅路径的集成，删掉几层性能只平滑下降而不崩塌。代价是引入分支结构，对早期硬件的内存访问不够友好，且深度带来的推理延迟是实打实的。回报则改变了一切：152 层网络（比 VGG 深 8 倍但计算量更低）、ImageNet 集成 3.57% 错误率、横扫 2015 年 ILSVRC 与 COCO 全部任务。
...
实践中可为 GAN 训练做健康诊断的三件套，比盯损失数字有效得多：
...
更值得保留的是其方法论：**一个架构的成功 = 论文创新 × 工程可复现性 × 部署友好度**。YOLO 每一代的论文贡献未必最惊艳，但"下载即可训练、导出即可部署"的体验让它赢了生态。这条经验放在今天的模型选型里依然成立。

截至 2026-09，这条线仍在快速演进：Ultralytics 的 YOLOv8/v11 把训练导出体验做到极致，[YOLOv12](https://arxiv.org/abs/2502.12524)（NeurIPS 2025）则用区域注意力（Area Attention）把 Transformer 的注意力机制塞进实时检测骨干，在保持 CNN 级推理速度的同时拿到注意力级的精度——注意力机制对 CNN 最后一块实时领地的反攻，值得跟踪；另一条线 RT-DETR 则把 DETR 系端到端检测做到了实时档。边缘设备上的检测选型，默认顺序可设为：先 YOLO 系打基线，精度不够再上两阶段或 DETR 系。
...
- **特征工程决定传统模型的上限**：树模型与线性模型时代，80% 的收益来自特征与标签口径；深度时代这部分工作变成了数据清洗与预训练语料构造——形式变了，"垃圾进垃圾出"没变。可采用的特征自检清单：
