...
title: 视频生成：时空建模的技术线
outline: [2, 3]
description: 从时空表示、生成主干与条件控制，到评估、部署和成本的系统化视频生成技术总览。
lastVerified: 2026-09-20
lastReviewed: 2026-09-26
reviewScope: 去噪阶段 MoE 的激活计算与权重显存分离
---

# 视频生成：时空建模的技术线

![视频生成技术全景：从技术演进、条件输入、文本编码、3D 因果 VAE、时空补丁、时空 DiT、迭代去噪到音画输出，并总览控制、训练、评估、加速、模型选型与有效成片成本](/images/ai/models/video-gen/video-generation-overview-b.png)

*总览图：本站生成的高清全文阅读地图，覆盖技术演进、生成主链路与工程决策；具体模型能力与参数以正文引用的一手来源为准。*

> 视频生成可以理解为在图像生成基础上增加时间维度，但这个新增维度会同时放大计算量、一致性和物理合理性问题。本文面向需要评估或落地视频生成能力的工程师，按“问题 → 表示 → 架构 → 控制 → 评估 → 部署与成本”组织。可核验事实优先引用官方文档、原始论文和官方仓库；工程建议单独标注适用边界。**资料核验日期：2026-09-20。**
...
- 经过 3D 因果 VAE 的时空压缩再切成时空补丁后，序列长度仍可能达到数万量级；精确数值取决于 VAE 的时空压缩比、patch 大小、帧数和分辨率。
- **不要把 API 的视频输入计费 token 与生成模型内部的潜空间 token 混为一谈。** 两者服务于不同目的，厂商也通常不会公开闭源模型的内部切分与压缩细节；本页只用可公开的模型结构或开源配置估算内部序列长度。
...
工程边界：短片段可依靠时空建模获得较稳定的局部一致性；长内容仍通常采用分镜生成、参考主体约束与后期剪辑。跨镜头角色一致性应通过参考图、关键帧或专门的身份条件进行验证，不能默认纯文生视频会保持角色身份。
...
结论：**Sora 技术报告的价值是给出统一表示与规模化训练的坐标系，而不是公开一份可复现实现。** 后续开源模型在 VAE 压缩、注意力结构、MoE 与少步生成等方向给出了可验证的工程实现；比较时应把 OpenAI 明确披露的事实与社区反推分开。
...
Wan2.2 把 LLM 已验证的 MoE 思路搬进视频扩散：A14B 系列用双专家按去噪阶段分工——**高噪声专家**负责早期（布局与构图），**低噪声专家**负责后期（细节与纹理），按信噪比阈值切换。总参数 27B、每步激活 14B，单步只使用对应专家，但权重存储仍要考虑两个专家；只有引擎卸载或分阶段加载时，才可能减少 GPU 驻留占用，同时增加传输或切换开销，验证损失曲线也显著优于单模型基线。这是"把模型做大"与"把成本控住"之间目前最实用的平衡点。
...
选型建议：文生/图生是基础能力，**参考主体与镜头控制更能区分内容生产可用性**。营销片、短剧与 IP 内容评估应提高“参考主体一致性”和“镜头控制”的权重，而不是只比较纯文生视频的单条画质。
...
工程边界：不要把特定模式下的最长生成时长等同于稳定的长片生产能力。通用长内容仍应按“多个可控短镜头 + 一致性约束 + 剪辑”设计，并用业务样本实测延展后的漂移与失败率。

## 开源生态架构对比（核验于 2026-09-20）
...
- **许可是硬约束，先看模型版本对应的许可证**。CogVideoX-2B 与 Wan2.1/2.2 的官方仓库给出了 Apache 2.0 口径；CogVideoX-5B、混元及其他自定义许可必须逐条核对地域、服务规模、再分发和商用条件，不能把“代码开源”自动等同于“模型权重可自由商用”。
...
选定开源模型只是开始，真正落地还要过几道工程关。以下按常见影响面排序：
...
评估原则：**VBench 适合做“能力剖面图”（哪一维强、哪一维弱），不应单独用于采购决策。** 原因有三：

1. 自动指标与人类偏好并不总一致，尤其美学、运动自然度这类主观维度。
2. 榜单容易被"针对维度调优"刷高，未必反映真实业务表现。
3. 你的业务提示词分布与 VBench 的 prompt suite 未必重合。

建议流程：**先用 VBench/FVD 形成能力剖面，再用覆盖真实业务分布的提示词集做盲测，统计“一次成片率”**（合格条数/生成条数）。测试集规模应按场景复杂度决定，20–30 条只能用于早期筛选，不能替代正式验收。第三方竞技场可以帮助建立候选池，但同样不能替代业务测试。
...
工程边界：免训练的缓存与量化通常是渐进式优化；蒸馏或更高压缩率可能带来更大的吞吐变化，也更可能改变画质分布。任何加速方案上线前都应以同一业务提示词集重测一次成片率，避免节省的单次推理成本被重跑抵消。
...
把图像提示词直接搬到视频，容易出现“画面动不起来”或“运动混乱”。根因是**视频提示词还要描述运动与镜头**。一个可复用的结构是“主体 + 运动 + 场景 + 镜头 + 风格”：
...
这套方法的价值在于：先通过受控测试确认提示词结构是否提升一次成片率，再决定是否更换模型。提升幅度取决于模型、任务和验收标准，不应引用脱离测试集的通用比例。

## 开源与闭源格局（核验于 2026-09-20）

### 可核验的代表模型

| 模型 | 一手来源可确认的要点 | 权重与许可 |
| --- | --- | --- |
| Sora 2 | OpenAI 系统卡将其定义为视频与音频生成模型，并披露能力与安全边界 | 闭源服务 |
| Veo 3.1 | Gemini API 文档明确支持原生音频、文生/图生、首尾帧、参考图与视频延展；分辨率和时长受模式约束 | 闭源 API |
| Wan2.2 A14B | 官方仓库公开 T2V/I2V 配置，支持 720p 与 480p 规格，采用稀疏专家路线 | 开放权重，Apache 2.0；仍需核对模型卡与可接受使用条款 |
| HunyuanVideo | 官方仓库给出 720p、129 帧与显存参考值，并使用腾讯混元社区许可 | 开放权重，自定义社区许可 |
| CogVideoX | 官方仓库公开 2B/5B 与 1.5 系列；2B 与 5B 的模型许可不同 | 部分 Apache 2.0，部分自定义许可 |
| LTX-Video / LTX-2 | 官方仓库已将 LTX-2 作为主线，强调同步音视频、多关键帧与高分辨率输出 | 开放代码/权重范围随版本变化，部署前查对应模型卡 |

这张表刻意不抄录第三方排行榜名次与聚合平台价格。它们更新频繁、口径易变，适合建立候选池，不适合作为长期正文中的事实锚点。采购或上线前，应重新核对官方 API 文档、模型卡、许可证、地区可用性与业务样本盲测结果。
...
flowchart TD
  ...
  O --> O1[Wan2.2 / HunyuanVideo / CogVideoX / LTX<br/>核对版本、许可与显存账]
  C --> C1[Sora / Veo 等托管服务<br/>核对最新能力、单价与数据条款]
...
视频 API 常按时长、分辨率、模型档位或生成次数计费；自建则按 GPU 卡时、存储、队列利用率与运维成本核算。价格变化快，因此正文不固化具体厂商报价，而保留可复用的测算公式：
...
例如一次成片率为 25%，仅生成环节的有效成本约为标价的 4 倍。比较闭源 API、托管开放权重和自建方案时，必须统一时长、分辨率、音频、并发和质量验收条件，并在同一业务提示词集上统计一次成片率。

把这笔账与“推理加速”一节连起来看：序列长度、去噪步数与模型规模都对应不同的优化手段，但最终应比较的是**质量门槛下的有效成片成本**，而不是单次调用价格。
...
规划边界：世界模型值得持续跟踪，但现阶段的视频生成收入与可验证需求仍主要来自内容生产（营销、短剧、影视预览、数字人）。技术规划可把“实时因果生成”作为兼顾当下实时内容与未来交互模型的能力，把“物理自洽”作为长期验收项；不应仅根据演示视频做投入决策。短期内，时空 DiT、视频 VAE、数据与推理优化仍是主要工程抓手。
...
- [Generate videos with Veo 3.1 in the Gemini API（Google AI 文档）](https://ai.google.dev/gemini-api/docs/veo) — Veo 3.1 的时长、分辨率、参考图、首尾帧、延展与原生音频参数（访问日期 2026-09-20）
...
- [Wan-AI/Wan2.2-T2V-A14B（Hugging Face 官方模型卡）](https://huggingface.co/Wan-AI/Wan2.2-T2V-A14B) — Wan2.2 A14B 权重、Apache 2.0 许可与模型文件（访问日期 2026-09-20）
- [tencent/HunyuanVideo（Hugging Face 官方模型卡）](https://huggingface.co/tencent/HunyuanVideo) — HunyuanVideo 权重、显存要求与腾讯混元社区许可（访问日期 2026-09-20）
- [zai-org/CogVideoX-2b（Hugging Face 官方模型卡）](https://huggingface.co/zai-org/CogVideoX-2b) — CogVideoX 2B 权重、运行规格与 Apache 2.0 许可（访问日期 2026-09-20）
- [zai-org/CogVideoX1.5-5B（Hugging Face 官方模型卡）](https://huggingface.co/zai-org/CogVideoX1.5-5B) — CogVideoX 1.5 5B 权重、规格与自定义许可入口（访问日期 2026-09-20）
- [Lightricks/LTX-Video（Hugging Face 官方模型卡）](https://huggingface.co/Lightricks/LTX-Video) — LTX-Video 权重、运行建议与版本许可证入口（访问日期 2026-09-20）
