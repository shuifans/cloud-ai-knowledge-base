...
reviewScope: CTC 时间戳适用边界与 TTS 互链说明
---

# 语音识别与理解

![ASR 管线、并列对齐方法与实时语音路线总览图](/images/ai/models/audio/audio-overview.png)
...
语音是重要的自然交互入口。这条技术线走过“声学模型 + 语言模型拼接”的经典时代，随后由端到端模型整合，并进入语音大模型阶段。工程评估不应只问“能不能用”，还要同时衡量延迟、可控性、准确率与合规。

**与姊妹篇的分工**：本文讲识别与理解；[语音生成](/ai/models/speech-gen)从公开 TTS 模型和论文说明声学表示、流匹配、声码器与流式服务边界。
...
部分存量系统中，电话客服、司法转写这些领域还有大量 Kaldi 系 HCLG 栈在跑。它们不新潮，但**热词注入、语法约束这些能力是即插即用的**，这是后面很多端到端模型至今没补齐的短板。经典管线的完整总结可看 Mohri 等人的 WFST 综述与 Hinton 等人的 DNN 声学模型综述（见参考资料）。
...
- CTC 的 token 尖峰可用于估计对齐，但不是天然精确的字边界；分词、帧步长与发射延迟会影响时间戳，字幕或强制对齐仍需单独验证。
...
**工程判断**：CTC 结构简单、推理一次前向就出结果（无自回归循环）、帧同步输出天然支持流式，至今仍是自监督模型（wav2vec 2.0 系）微调头和多任务联合训练（CTC/attention 混合）的默认选择；但它单独作为产品级 ASR 主干已让位于 RNN-T 和 AED——多数场景工程上常见的情况是：CTC 做辅助损失提稳训练，而不是做最终解码器。
...
Whisper 论文显示，大规模、多样化弱监督数据可以显著提升跨数据集零样本泛化；但“逼近人类”只适用于论文给定数据集和口径，不能外推到所有语言、口音或专业术语。Whisper large 系在会议、播客和电话音频上的表现仍需使用目标领域数据，并与专用模型在相同切分、VAD 和文本归一化规则下比较。
...
- **落地边界**：纯自监督微调的 ASR 在中文工业场景不能默认优于 Whisper 大模型或商用引擎——自监督的主战场是**没有标注数据的长尾语言**和**表征复用**，而不是和成熟引擎拼中文 WER。
...
对客服、外呼等**流程确定、话术受控**的场景，级联方案通常更容易审计、替换和单点优化；陪伴、助手、口语练习等**体验驱动**场景可以评估端到端方案，但应单独验证延迟、打断、幻觉和工具调用可靠性。混合形态由端到端模型负责听说，关键动作经函数调用落回受控流程，是一种可验证的工程折中，而非对所有场景都成立的“主流答案”。
...
- **代码转换（code-switch，中英混说）单独测**：混说句子里英文词的 WER 通常是纯中文的 2–3 倍（需在业务数据上实测），模型清单标"支持英语"不等于"支持混说"，要用真实业务里的混说样本单独建测试集。
...
- [Whisper large-v3（Hugging Face）](https://huggingface.co/openai/whisper-large-v3) — OpenAI 官方 ASR 权重与 Apache-2.0 许可证；模型卡标注 99 种语言，但各语言、口音和领域的错误率并不一致
- [Whisper large-v3-turbo（Hugging Face）](https://huggingface.co/openai/whisper-large-v3-turbo) — 官方加速版本模型卡；质量、速度与长音频切分策略需单独评测
- [Qwen2-Audio-7B-Instruct（Hugging Face）](https://huggingface.co/Qwen/Qwen2-Audio-7B-Instruct) — Qwen 官方音频语言模型卡、权重与使用示例
- [SenseVoiceSmall（Hugging Face）](https://huggingface.co/FunAudioLLM/SenseVoiceSmall) — FunAudioLLM 官方多语种语音识别、语言/情感与音频事件识别模型卡

> ASR 模型卡上的公开 WER 通常来自特定数据集与解码配置。生产结论必须记录语言、口音、信噪比、采样率、切分/VAD、热词和归一化规则，不能跨口径直接比较。
