...
reviewScope: 向量降维的存储口径、知识更新与权限撤销链路
---
...
RAG 质量取决于解析、切分、检索、重排、生成与权限治理的共同作用。应利用带出处的评测集分段归因，不能在没有项目样本时断言“多数失败都发生在切分”。
...
**块大小—召回—成本的三角**是切分决策的本质：块调大，单个块语义可能被稀释且生成 token 成本上升；块调小，检索更精细但上下文容易碎片化。可以把 **256–1024 token、10%–20% 重叠**作为实验起点，结构化文档还应比较按章节切分。真正判据是目标评测集表现，而不是任何默认值。
...
嵌入选型应同时验证语言与领域、许可、部署方式和召回效果。Matryoshka 模型可支持截断到指定维度，但并非任意 embedding 都能直接截断。1024 维降到 256 维只把原始向量的浮点存储减为四分之一；图索引、元数据、过滤结构和副本不同比例缩小，质量变化需要业务评测。
...
榜单与现成模型都不够时，可以评估**领域对比微调**：用业务中的 `(query, 相关 chunk)` 对训练开放权重嵌入模型。是否启动应由基线差距、标注质量与投入产出决定，不能套用固定的“千级标注对”门槛；微调后需进行版本管理，并保留通用评测集防止领域过拟合。通常应先验证换模型与改切分，微调不是默认动作。
...
触发策略上建议**条件触发而非全量开启**：多轮对话中存在未消解指代才做会话补全；检索置信度低（top-1 分数低于阈值）或意图分类为复合问题时才触发改写与 multi-query；HyDE 只在"查询过短且首轮召回为空"的分支启用。全量开启的代价不只是钱——**改写本身会引入错误**（把"E-107"改写成"温度故障"反而丢掉字面锚点），条件触发把改写的风险面限制在它确实有收益的子集上。阈值与触发规则同样进评测集做 A/B，这是查询侧优化与"拍脑袋加技巧"的分界线。
...
2024 年之后还有第三条检索路：**学习型稀疏向量**（SPLADE、bge-m3 的 sparse 输出这类）——用模型学出"扩展后的词项权重"，既保留倒排索引的字面匹配与精确词优势，又具备一定的同义扩展能力（模型可以给文档中没出现的词项赋权）。工程上的吸引力在于：bge-m3 这类模型一次前向同时产出稠密与稀疏两路向量，**混合检索不需要额外维护一套 BM25 分词器与全文索引**，对中文分词敏感的场景（专有名词切分错误）尤其省事。工程建议：有现成全文索引（Elasticsearch/OpenSearch 类）就用 BM25，新建系统且选了一体化模型时可以学习型稀疏替代 BM25，两者与稠密向量的融合方式（RRF）不变。
...
多数场景可从 RRF 起步，再依据评测决定是否调整：它的收益来自"两路互为冗余"，调参空间小是优点不是缺点。
...
工程建议：**优先上 Cross-Encoder**。它是为相关性专门训练的，性价比远高于拿通用 LLM 做重排；LLM 重排留给"必须理解复杂意图"的少数场景。2026 年 Qwen3-Reranker（0.6B/4B/8B，32K 上下文）与 Cohere Rerank 3.5（4K、多语言）是两条主流路线。
...
在合规场景中，第 2、4 条尤其重要：**“部分回答 + 显式缺口”**通常比全有全无的拒答更易用，而**冲突并列**能把制度版本冲突等语料治理问题显式暴露出来，避免模型在缺少依据时自行选边。
...
工程建议：索引记录文档版本、有效时间、抓取时间、权限版本和来源。新知识替换旧内容时，同步更新父子块、向量、关键词索引及引用；删除与撤权还要使答案缓存、会话和长期记忆失效。不能只新增向量而保留所有过期答案。检索返回的指令属于数据，工具执行仍需独立授权，详见[Agent 安全与可靠执行](/ai/agent/security)。
...
固定管道是这段代码在“plan 恒为单步检索、judge 恒为通过”时的特例。Agentic RAG 可拆成三个可度量问题：**plan 的准确率**、**judge 的校准度**与**单请求工具调用数分布**。应先在评测集上测量这三个量，再决定自主度，而不是先选框架再补评测。
...
**2025–2026 的产品形态印证了这个方向**：检索已经从“管道阶段”变成“智能体工具”。OpenAI Deep Research、Gemini Deep Research 与 Perplexity Deep Research 等产品，都采用模型规划检索路径、多轮获取信息并产出带引用报告的形态；评测侧也出现了 BrowseComp 这类“答案易验证但难找到”的深度检索基准。对企业 RAG 的启示是：**私有知识库可以作为工具接入智能体编排**，固定管道则成为其中一个可调用、可降级的路径。更稳妥的工程划分是：高频问答优先采用优化后的固定管道，以控制延迟与成本；研究、分析类的低频复杂问题再采用 Agentic 编排。该划分是架构建议，不代表所有产品的统一结论。
...
因此可将引入判据写成 checklist：**评测集中多跳/全局类问题占比是否显著且当前方案无法回答 → 加入父子块/摘要块后是否仍无改善 → 语料变更频率是否允许定期重建 → 索引与查询的 token 预算是否单独核算**。只有四项均满足时再考虑上线；其中“占比阈值”应由业务评测集确定，不宜套用固定经验值。默认优先把混合检索与重排做扎实。
...
把 CAG 单独说透，因为它是 2025–2026 年容易被误用的中间形态：CAG（Cache-Augmented Generation）把知识库内容预置到长上下文，并尽量复用模型侧 KV cache 或供应商的 prompt cache；它不是一个跨厂商都能“永久预热整库”的统一产品能力。它的适用边界较窄：**语料总量需小于有效上下文容量、更新频率需低到缓存失效与重建可接受、且权限隔离方案必须明确**。更适合的形态通常是规模受控、权限一致的知识集；引入多租户后往往需要隔离缓存，存储、更新和失效管理成本会明显上升。
...
工程上应**把“长上下文 vs RAG”当成每个功能的局部决策，而不是系统级站队**；上下文工程还应同时考虑缓存命中率、上下文编排、检索与注入的混合策略，RAG 是其中一环而非全部。
...
最后补一层离线与在线的关系，避免"评测集分数高了就等于线上好了"的错觉：**离线评测集回答的是"这次改动有没有回归"，在线指标回答的是"评测集有没有覆盖真实分布"**。两者出现背离（离线涨、线上负反馈涨）通常意味着评测集分布过时了——比如业务新增了某类问题而评测集没补。维护节奏可设为：每次链路变更跑离线回归（分钟级、自动），每周做一次线上抽样归因并把新坏例回流入评测集（小时级、半自动），每季度复盘一次评测集的类别分布与真实查询日志的偏移。评测集不是验收材料，是这套系统的回归测试套件，它的维护投入应当写进运营预算。
...
**“重排候选数 × QPS”与索引重建**是两类容易漏算的成本，应在容量规划阶段写入预算表并通过压测校准。
