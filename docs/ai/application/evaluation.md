...
reviewScope: 移除无来源工具收益倍数，补充模型升级与安全回归口径
---

# 大模型评测：从基准到生产监控

![公开基准、裁判评测与生产监控闭环图](/images/ai/application/evaluation/llm-evaluation-overview.png)
...
| 综合偏好 | LMArena（原 Chatbot Arena） | 匿名两两对战、众包人类偏好、Elo 排名 | 在线新题可降低固定题库污染，但不能保证完全未见；需关注数据泄漏与重复题；但更测"讨喜程度"而非正确性，存在"榜单幻觉"争议 |
...
随着 GPQA Diamond 上高分模型增多，HLE（Humanity's Last Exam）等更困难的专家题基准被用于补充区分度。但不能据此断言知识基准都在一年内饱和；选型时应同时检查基准发布日期、污染风险、题目覆盖面、模型快照和独立复现，而不是只看总分。
...
长上下文选型至少应测试三类任务：单点检索、多点聚合、长文总结后追问；不能只用“捞针”结果作结论。
...
需要明确：**公开基准的分数 ≠ 业务可用性**。公开基准更适合快速淘汰明显弱的候选。选型与上线判断仍应落在自建评测集上；自建集应来自实际业务分布，并通过隔离、版本管理和污染检查降低“刷榜”影响。
...
一种典型情况是：候选模型在公开代码 agent 榜单表现较好，但在含私有依赖的真实 issue 集上明显下降。差距可能来自 harness、依赖与任务分布，因此公开基准只适合第一轮筛选。
...
- **定期与人工对齐**：按风险和流量抽样做人工双标，计算裁判与人工的一致率；上线门槛应由任务代价和置信区间确定，不统一套用 80%
...
裁判提示词会显著影响偏差。下面给出一个绝对打分模板（要点：角色与任务分离、固定 rubric 锚点、强制先证据后结论、结构化输出）：
...
两两对比模板还应交换位置运行，结论不一致时记为“平局”，并避免把无关的长度与排版当成质量。要求“先证据后结论”有助于降低直觉式偏袒，但仍需用人机一致率验证。
...
小样本上的几个百分点差异可能落在统计噪声内；比较版本时应采用同题配对设计并报告置信区间。每层题量和全集规模应由目标最小可检测差异、方差与置信水平计算，不能把固定题量当作统一标准。

报告数字时应**附置信区间或离散度，而不是只给裸平均分**。裁判打分报告均值时附标准差，两两对比报告胜率时附交换位置后的一致率。

### 分场景评测要点

**选型评测**：可复制流程是先用公开基准淘汰明显不达标者，再把少量候选放入自建黄金集做**同卷同判**回归，并将单位质量成本、P95 延迟与合规约束纳入决策。候选是否“打平”应依据样本量、置信区间和业务最小可感知差异确定，不能统一套用 3 个百分点阈值。
...
| 指标层 | 指标 | 判据 |
| --- | --- | --- |
| 结果 | 任务成功率 | **以终态验证为准**（查数据库、查文件、查工单状态），同时验证权限与副作用约束 |
| 过程 | 轨迹质量 | 工具调用序列与期望路径比对：精确匹配、顺序匹配或无序匹配三档松紧 |
| 过程 | 工具调用正确率 | 分别核对工具、参数、权限与结果；不以未注明任务集的提升倍数作保证 |
...
告警阈值可从统计基线起步：以最近 4 周同意图层的裁判质量分为基线，取均值 μ 与标准差 σ，例如用 μ−2σ 触发复核、μ−3σ 或连续越线触发阻断；具体阈值必须通过历史回放校准。抽样量按统计判据倒推，样本不足时只进入人工复核。输入分布与评测集偏离时，应先补充评测样本再解释质量分。
...
- **过度拒绝的代价**：安全调优过头会伤可用性。安全评测集与能力评测集应**同批运行、同版本比较**；安全指标改善若伴随拒答率上升，需要计算净收益
...
红队执行节奏可按风险分层：大版本上线前进行人工与自动化组合红队，日常迭代运行自动化攻击集回归；攻击集与业务对抗题统一版本管理，新增攻击手法入库后立即回归。
...
过程侧的代表工作：OpenAI 的 Process Reward Model 研究（arXiv:2305.20050）在其数学任务设置中显示，过程监督奖励模型的 best-of-N 验证效果优于结果监督与多数投票（下图）；ProcessBench（arXiv:2412.06559）测“能否定位推理链中最早出错的步骤”；PRMBench（arXiv:2501.03124）进一步提高过程级判分难度。工程含义是：过程级裁判可定位“从哪一步开始出错”；相较只看最终答案，它通常能提供更直接的诊断信号，但效率提升幅度应以实际评测流程为准。
...
工程建议：每次模型或工具升级，保存模型请求 ID、实际服务版本、提示词与数据集版本、推理档位、工具配置、运行日期及费用口径。别名可能路由到新模型，即使请求字符串不变，结果也不一定可复现。

对同一批任务做成对比较，同时报告成功率的分子/分母、重复运行策略、失败类别、P95/P99 与每成功任务成本。轨迹可存在多条正确路径；只要求必须的安全约束、关键顺序和业务终态，避免把唯一示范轨迹当作全部正确答案。安全集还要覆盖正常任务的误拒绝率，样本比例按风险与真实流量设定，不把固定 10–20% 当成标准。
