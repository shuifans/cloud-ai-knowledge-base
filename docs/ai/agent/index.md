...
![从 Copilot 到 Agent 的分级、架构与记忆分工图](/images/ai/agent/agent-overview.png)
...
- **长期记忆**：外部化存储、跨会话持久。三种常见载体：**向量库**（语义召回，适合偏好与经验）、**KV 存储**（结构化事实，如用户画像，KV 即 key-value 键值对）、**文件/知识库**（沉淀式文档，如 AGENTS.md 式的笔记）。MemGPT 开创过"OS 分页"路线——模型自主换入换出记忆层级，但多数工程案例更朴素：写入与检索由编排层控制，模型只消费注入结果。
...
总结触发建议：①**占用逼近窗口阈值**——达到窗口的 70%–80% 就压缩，给摘要调用本身与后续生成留出预算；②**话题切换**——旧话题完整、语义自洽时压缩，质量最高；③**任务收尾**——会话关闭前把结构化结论写入长期记忆。相形之下，"固定每 N 轮"是最差的触发条件，它会把语义完整的片段拦腰截断。另一条来自 Anthropic 工程实践的告诫：把压缩提示词当生产系统对待，优化目标先保召回再保精简——压缩后丢一个关键决策，远比多留几句废话致命。
...
- **写入时机**：每轮写还是会话末写，两派都有。建议偏好、决策等结构化事实每轮结束即写（会话中断不丢、可即刻复用），原始轨迹在会话末批处理。每轮写就必须同时做去重与合并，否则三个月后记忆库里全是重复事实。
