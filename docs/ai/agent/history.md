...
![Agent 阶段演进与能力、期望、成本关系图](/images/ai/agent/history/history-overview.png)
...
本文将七十年历程分为六个阶段，每阶段用同一组三问展开（发生了什么 / 为什么是这时 / 留下了什么），末尾再统一分析“期望落差”与规律：
...
一个读法提醒：编年史最容易写成流水账。本文在每阶段末尾放一句"遗产"判断——因为对做架构的人而言，**事件本身不重要，事件沉淀下来的接口、协议与失败教训才重要**。不同角色可以各取所需：做技术选型的先读"落差与节律"和"实践与选型"两章；做协议与集成的重点看平台期的接口事件表与 MCP 时序图；给管理层写汇报的直接引用三次寒冬表与 Gartner 口径；只想快速补齐 2024–2026 的读者可以直奔平台期与长程期——原"热点编年史"的材料在这两章全量保留。
...
- 工程判断与可验证事实分开书写；缺少独立证据的观点明确标为建议，不冒充普遍结论
...
**Siri 时代为什么做不成通用 agent**，可归纳为三个“没有”：没有开放式推理能力（意图分类难以覆盖未见表达）、没有统一行动空间（每个技能都要单独对接）、没有成熟的错误治理机制。LLM、function calling 与协议标准化分别缓解了这些限制，但没有彻底消除可靠性问题。
...
ReAct 论文里更值得关注的一组结论不是分数，而是定性差异：纯 CoT 会"自信地编造"（幻觉无法自查），纯 Act 会"盲目试错"（缺乏规划），ReAct 用外部观察给推理装上**可验证的刹车**。这个设计原则——让 agent 的每一步都能被外部事实校正——是今天所有可靠 agent 系统的底色。
...
ReAct 循环成为此后大量 agent 的常见骨架：在 Claude Code、Codex、Deep Research 等产品中可以看到“推理—调用工具—观察—继续”的相似结构。下面的决策图给出一种评审 agent 方案的判断入口：
...
MCP 的机制一句话讲清：三个角色——host（agent 应用，如 IDE 或客户端）、client（host 内的连接会话）、server（能力提供方）；server 对外暴露三类原语——**tools**（可调用的函数）、**resources**（可读的数据）、**prompts**（预置模板）；传输层本地走 stdio、远程走 HTTP。它的工程含义是把 M 个 agent × N 个工具的集成问题变成 M + N：每边只实现一次协议。工程上常见的情况是：2025 年企业落地 agent 时，MCP 解决的是"连得上"，随之而来的治理问题（谁能暴露什么 tool、调用如何审计）催生了 MCP 网关这个新品类——协议标准化永远只是把问题往上推一层，而不是消灭问题。
...
到 2026 年回看，agent 第一个跑通商业账本的是编码：Claude Code 订阅制（20–200 美元/月档）+ API 双轨，2026 年上半年年化收入达数十亿美元量级（公开报道口径）。为什么是编码？可归纳为四条，全部与"可靠性三角"有关：
...
2025 下半年起，agent 任务的长度从"几十次工具调用"进入"数百次工具调用"量级，瓶颈从模型智力转向**上下文管理**：上下文是有限资源，塞满即退化。Anthropic 2025-09-29 的工程博客《Effective context engineering for AI agents》把实践收敛为压缩（compaction）、结构化笔记、子 agent 隔离、按需检索几招；Agent Skills 的"按需加载"本质上也是上下文预算分配。工程上常见的情况是：长程 agent 项目的返工，八成发生在上下文策略而不是模型选型上。
...
这四种方法的共同本质是**把上下文当预算管理**：每一步都要判断信息值得占用多少 token、保留多久。长程 harness 的实现和默认策略不同，选型时应直接比较上下文压缩、持久化、检索与失败恢复机制，而不是只看宣传语。

### 多 agent 协作的产品化

单 agent 执行长程任务会遇到上下文与失败隔离限制，因此编排者—工作者模式逐渐进入产品：主 agent 派生子 agent 并回收结果，LangGraph、Microsoft Agent Framework、ADK 等框架也把子 agent、检查点和人机审批位做成一等能力。多 agent 的主要工程收益通常来自**隔离**（上下文预算、故障域、权限面），而不是未经验证的“群体智慧”；实际收益仍需用同任务、同预算评测确认。框架层的详细对比见 [Agent 开发框架对比](/ai/agent/frameworks)。
...
企业评估个人 agent 类工具时应检查三类风险：**默认权限是否过宽**、**企业数据是否离开受控边界**、**聊天渠道是否具备高风险操作所需的审批语义**。这不是对特定项目作安全定性，而是在区分个人生产力工具与企业可管控 agent 的设计约束。
...
关于“主动开源”还是“竞争推动”的动机，公开信息不足以给出确定结论。更可验证的结果是：执行引擎与框架的开放提高了模型、运行时与连接器分别选型的可行性。详见 [Agent 开发框架对比](/ai/agent/frameworks)。
...
这张图采用的评审顺序是：先看可靠性，再看错误经济学，最后看接口成熟度。只有三项均满足要求时再扩大自主权，否则降级为 workflow 或增加护栏。“workflow 为主、agent 为辅”是一种风险优先的默认策略，仍需按场景验证。
...
选型之后、上线之前，上线前至少应回答四个问题，缺一不可：**成功率基线**（在自建任务集上跑 50 例，记录成功率/步数/成本三指标，任务集小于 30 例的评估不作数）；**失败模式清单**（把失败样例归类到循环/上下文/权限/接口四类，确认每一类都有对策）；**回滚路径**（agent 造成的每一个副作用是否可撤销，不可撤销的是否都挂了审批位）；**退出条件**（模型涨价、协议变更、成功率退化到阈值以下时，切换到备选形态的成本是多少）。这四问的本质是把"agent 是实验品还是产品"区分开：答不上来的，就还是实验品。
